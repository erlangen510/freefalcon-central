...
void AssignACOrientation(Trotation *&orientation, Tpoint &origin,
                         Tpoint &position, Tpoint &worldPosition, bool right);
...
float wingvortexAlphaHigh =
    20; //RV - I-Hawk - AOA above this value enough to generate wingtip vortex trails
float wingvortexAlphaLimit =
    29.5; //Above this value stop as probably AC is stalling
float wingvtxTrailSizeCx =
    1; //RV - I-Hawk added factors to suppoert RV new trails size/alpha changes
...
float engineTrailMargin =
    1; //I-Hawk - added to support fade in/out of engine smoke trails with altitude
float engineTrailRPMFactor =
    1; //I-Hawk - added to support fade in/out of engine trails with RPM
...
void AircraftClass::ApplyDamage(FalconDamageMessage *damageMessage)
{
    ...
    object =
        (SimBaseClass *)vuDatabase->Find(damageMessage->dataBlock.fEntityID);
    ...
    if (damageMessage->dataBlock.damageType ==
        FalconDamageType::ObjectCollisionDamage)
    {
        ...
        relVel = (float)sqrt(relVec.x * relVec.x + relVec.y * relVec.y +
                             relVec.z * relVec.z);
    ...
    else if (damageMessage->dataBlock.damageType ==
             ...
             FalconDamageType::MissileDamage)
    {
        ...
        {
            JoystickPlayEffect(JoyHitEffect,
                               FloatToInt32((float)atan2(ry, rx) * RTD));
        }
    }
    ...
    if (IsLocal() and not IsSetFalcFlag(FEC_INVULNERABLE))
    {
        ...
        */
        ...
        case 0: // Back, Left, Upper
            failuresPossible =
                (1 << FaultClass::eng_fault) + (1 << FaultClass::eng2_fault) +
                (1 << FaultClass::hud_fault) + (1 << FaultClass::fcc_fault) +
                (1 << FaultClass::flcs_fault) + (1 << FaultClass::mfds_fault) +
        ...
        case 1: // Front, Left, Upper
            ...
            if (g_bRealisticAvionics and g_bNewDamageEffects)
            {
                ...
                if (rand() % 100 < 25 and
                    not LEFState(LT_LEF_OUT)) //25% we're taking damage
                {
        ...
        case 3: // Front Right, Upper
            ...
            if (g_bRealisticAvionics and g_bNewDamageEffects)
            {
                ...
                if (rand() % 100 < 25 and
                    not LEFState(RT_LEF_OUT)) //25% we're taking damage
                {
        ...
        default:
            ...
            if (g_bRealisticAvionics and g_bNewDamageEffects)
            {
                    ...
                    not LEFState(RT_LEF_OUT)) //25% we're taking damage
                {
                    ...
                    {
                        LEFOn(RT_LEF_OUT);
                        RTLEFAOA = GetDOFValue(IsComplex() ? COMP_LT_LEF :
                                                             SIMP_LT_LEF);
                    }
                    else
                    {
                        LEFOn(RT_LEF_DAMAGED);
                        RTLEFAOA = GetDOFValue(IsComplex() ? COMP_RT_LEF :
                                                             SIMP_RT_LEF);
...
int AircraftClass::SetDamageF16PieceType(DamageF16PieceStructure *piece,
                                         int type, int flag, int mask,
                                         float speed)
{
    if (not(type bitand mask))
        return 0;
...
int AircraftClass::CreateDamageF16Piece(DamageF16PieceStructure *piece,
                                        int *mask)
{
    ...
    {
        if (p > range2)
            damagetype or_eq 0x1c;
        else if (p > range1)
            damagetype or_eq 0x8;
        else
            damagetype or_eq 0x4;
    }

    float speed = (float)sqrt(XDelta() * XDelta() + YDelta() * YDelta() +
                              ZDelta() * ZDelta());
    ...
    if (damagetype bitand 0x10)
    {
        ...
        if (damagetype bitand 0xc) // damage to the front
        {
            ...
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_LWING, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_LSTAB, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_RWING, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_RSTAB, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_NOSE, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_FRONT, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_BACK, dirflag, *mask, speed);
        ...
        else if (damagetype bitand 0x3)
        {
            ...
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_RSTAB, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_ONLYBODY, dirflag, *mask, speed);
    ...
    else
    {
        if (damagetype bitand 0x1)
        {
            ...
            if (damagetype bitand 0x4) // damage to the nose
            {
                ...
                numpiece += SetDamageF16PieceType(
                    &(piece[numpiece]), DAMAGEF16_NOSE, dirflag, *mask, speed);
                numpiece += SetDamageF16PieceType(&(piece[numpiece]),
                                                  DAMAGEF16_NOLEFTANDNOSE,
                                                  dirflag, *mask, speed);
            ...
            else
            {
                ...
                numpiece += SetDamageF16PieceType(
                    &(piece[numpiece]), DAMAGEF16_LSTAB, dirflag, *mask, speed);
                numpiece +=
                    SetDamageF16PieceType(&(piece[numpiece]), DAMAGEF16_NOLEFT,
                                          dirflag, *mask, speed);
        ...
        else if (damagetype bitand 0x2)
        {
            ...
            else
            {
                ...
                numpiece += SetDamageF16PieceType(
                    &(piece[numpiece]), DAMAGEF16_RSTAB, dirflag, *mask, speed);
                numpiece +=
                    SetDamageF16PieceType(&(piece[numpiece]), DAMAGEF16_NORIGHT,
                                          dirflag, *mask, speed);
        ...
        else if (damagetype bitand 0x4)
        {
            ...
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_NOSE, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_NONOSE, dirflag, *mask, speed);
        ...
        else if (damagetype bitand 0x8)
        {
            ...
            numpiece += SetDamageF16PieceType(
                &(piece[numpiece]), DAMAGEF16_FRONT, dirflag, *mask, speed);
            numpiece += SetDamageF16PieceType(&(piece[numpiece]),
                                              DAMAGEF16_BACKWITHWING, dirflag,
                                              *mask, speed);
    ...
    if (not numpiece) // if no damage, randomized damage parts
    {
        ...
        for (k = 0; k < 7; k++)
        {
            if (l bitand j)
            {
                numpiece += SetDamageF16PieceType(&(piece[numpiece]), l,
                                                  dirflag, *mask, speed);
            }
        ...
        numpiece +=
            SetDamageF16PieceType(&(piece[numpiece]), i, dirflag, *mask, speed);
        *mask and_eq i;
    }
...
// Visual debris only; vehicle destruction and damage remain in RunExplosion/ApplyDamage.
    ...
    SimBaseClass *tmpSimBase =
        new SimStaticClass(Type()); //new SimBaseClass(Type());
    tmpSimBase->SetYPR(Yaw(), 0.0f, 0.0f);
    CalcTransformMatrix(tmpSimBase);
    OTWDriver.CreateVisualObject(tmpSimBase, piece->damage, &piececenter,
                                 (Trotation *)&tmpSimBase->dmx,
                                 OTWDriver.Scale());
...
void AircraftClass::RunExplosion(void)
{
    ...
    {
        Tpoint vec = {XDelta(), YDelta(),
                      ZDelta()}; //I-Hawk - Added vector for AC explosion effect
...
void AircraftClass::AddEngineTrails(int ttype, DWORD *tlist, DWORD *tlist_trail)
{
    ...
    vec.x = orientation->M11 * thrx + orientation->M12 * thry +
            orientation->M13 * thrz;
    vec.y = orientation->M21 * thrx + orientation->M22 * thry +
            orientation->M23 * thrz;
    vec.z = orientation->M31 * thrx + orientation->M32 * thry +
            orientation->M33 * thrz;
    ...
    */
    ...
    if (tlist == conTrails or tlist == colorConTrails)
    {
        ...
        case 2:
            if (fabs(af->auxaeroData->engineLocation[0].y -
                     af->auxaeroData->engineLocation[1].y) < 9.0f)
            {
                ContrailsLocation[0].x = af->auxaeroData->engineLocation[0].x;
                ContrailsLocation[0].y =
                    (af->auxaeroData->engineLocation[0].y +
                     af->auxaeroData->engineLocation[1].y) /
                    2.0f;
            ...
            //
        case 3:
            if (fabs(af->auxaeroData->engineLocation[1].y -
                     af->auxaeroData->engineLocation[2].y) < 4.5f)
            {
                if ((Get3DDistance(af->auxaeroData->engineLocation[1],
                                   af->auxaeroData->engineLocation[2])) < 4.5f)
                {
                    nContrails = 1;
        ...
        case 4:
            nContrails =
                0; //make sure we get this changed in one of the conditions below...

            if (fabs(af->auxaeroData->engineLocation[0].y -
                     af->auxaeroData->engineLocation[1].y) < 9.0f)
            {
                if ((Get3DDistance(af->auxaeroData->engineLocation[0],
                                   af->auxaeroData->engineLocation[1])) < 9.0f)
                {
                    nContrails = 2;
                    ContrailsLocation[0] =
                        Get3DMiddle(af->auxaeroData->engineLocation[0],
                                    af->auxaeroData->engineLocation[1]);
                    ContrailsLocation[1] =
                        Get3DMiddle(af->auxaeroData->engineLocation[2],
                                    af->auxaeroData->engineLocation[3]);
                }
            }

            if (fabs(af->auxaeroData->engineLocation[1].y -
                     af->auxaeroData->engineLocation[2].y) < 9.0f)
            {
                if ((Get3DDistance(af->auxaeroData->engineLocation[1],
                                   af->auxaeroData->engineLocation[2])) < 9.0f)
                {
                    if ((Get3DDistance(af->auxaeroData->engineLocation[0],
                                       af->auxaeroData->engineLocation[3])) <
                        9.0f)
                    {
                        nContrails = 1;
                        ContrailsLocation[0] =
                            Get3DMiddle(af->auxaeroData->engineLocation[1],
                                        af->auxaeroData->engineLocation[2]);
                    }
                    else
                    {
                        nContrails = 3;
                        ContrailsLocation[0] =
                            af->auxaeroData->engineLocation[0];
                        ContrailsLocation[1] =
                            Get3DMiddle(af->auxaeroData->engineLocation[1],
                                        af->auxaeroData->engineLocation[2]);
                        ContrailsLocation[2] =
        ...
        nTrails =
            nEngines; // in case of regular engine smoke, we keep a trail for each engine
    ...
    for (int i = 0; i < nTrails; i++)
    {
        ...
        pos.x = orientation->M11 * (tp->x + offset) + orientation->M12 * tp->y +
                orientation->M13 * tp->z;
        pos.y = orientation->M21 * (tp->x + offset) + orientation->M22 * tp->y +
                orientation->M23 * tp->z;
        pos.z = orientation->M31 * (tp->x + offset) + orientation->M32 * tp->y +
                orientation->M33 * tp->z;
        ...
        if (tlist not_eq conTrails)
        {
            if (tlist not_eq colorConTrails)
            {
                ...
                engineTrailAlpha =
                    (((5000.0f - (-ZPos())) / 5000.0f) * 0.25f + 1) *
                    engineTrailMargin * engineTrailRPMFactor;
        ...
        else
        {
            ...
            {
                engineTrailMargin *=
                    (-ZPos() - contrailLow90Percent) / contrailLow10Percent;
            }

            if (-ZPos() > (contrailHighValue) and
                -ZPos() < contrailHigh110Percent)
            {
                engineTrailMargin *= (fabs(-ZPos() - contrailHigh110Percent)) /
                                     contrailLow10Percent;
            }

            engineTrailAlpha =
                engineTrailMargin; //Really means (1 * EngineTrailMargin) ...
            engineTrailSize = 1;
        }
        ...
        tlist_trail[i] = DrawableParticleSys::PS_EmitTrail(
            tlist_trail[i], tlist[i], pos.x, pos.y, pos.z, engineTrailAlpha,
            engineTrailSize);
    }
}
...
void AircraftClass::ShowDamage(void)
{
    ...
    if (not contrailLowValue) // Make sure I assigned contrails weather values only once
    {
        ...
        contrailLow10Percent =
            (((WeatherClass *)realWeather)->contrailLow * 0.1f);
        contrailLow90Percent =
            (((WeatherClass *)realWeather)->contrailLow * 0.9f);
        contrailHigh110Percent = (((WeatherClass *)realWeather)->contrailHigh +
                                  contrailLow10Percent);
    }
    ...
    if (not damageTrailLocationSet)
    {
        ...
        case 1:
            damageTrailLocation0 = damageTrailLocation1 =
                af->auxaeroData->engineLocation[0];
            break;
        ...
        damageTrailLocationSet =
            true; //do not try to set damage location more than once...
    }
    ...
    if (not OnGround() and af->EngineTrail() >= 0)
    {
    ...
    if (-ZPos() > contrailLow90Percent and -ZPos() < contrailHigh110Percent)
    {
    ...
    if (af->auxaeroData->wingTipLocation.y not_eq 0 and
        af->auxaeroData->wingTipLocation.y < 500 and GetKias() > 80)
    {
        ...
        if ((OTWDriver.renderer and
             ...
             -ZPos() > minwingvortexalt and -ZPos() < maxwingvortexalt) and

            ((currentAOA > wingvortexAlpha and currentG > wingvortexgs) or
             (currentAOA > wingvortexAlphaHigh and
              currentAOA < wingvortexAlphaLimit and
        ...
        if (OTWDriver.renderer and currentG > vortexG and vortexCondition and
        ...
        if (OTWDriver.renderer and currentAOA > vortexAOA and
            currentAOA < vortexAOALimit and vortexCondition and
            -ZPos() < vortexMaxaltLimit)
        {
            doAOAVortex = 1;
        }
        ...
        if (dotrails or doturb)
        {
            ...
            if (orientation)
            {
                ...
                if (af->auxaeroData->hasSwingWing)
                {
                    ...
                    tp.x = x * cos(swingWingAngle) - y * sin(swingWingAngle) +
                           af->auxaeroData->swingWingHinge.x;
                    tp.y = x * sin(swingWingAngle) + y * cos(swingWingAngle) +
                           af->auxaeroData->swingWingHinge.y;
                }
                ...
                if (dotrails)
                {
                    ...
                    if (currentVCAS < 250 and currentVCAS > vortexMinSpeedLimit)
                    {
                        wingvtxTrailAlphaCx *=
                            (currentVCAS - vortexMinSpeedLimit) /
                            (250.0f - vortexMinSpeedLimit);
                    }

                    float wingtipTrailAltCx =
                        1.0f - (-ZPos() / maxwingvortexalt);
                    ...
                    lwingvortex_trail = DrawableParticleSys::PS_EmitTrail(
                        lwingvortex_trail, lwingvortex, ltip.x, ltip.y, ltip.z,
                        wingvtxTrailAlphaCx, wingvtxTrailSizeCx);
                    ...
                    rwingvortex_trail = DrawableParticleSys::PS_EmitTrail(
                        rwingvortex_trail, rwingvortex, rtip.x, rtip.y, rtip.z,
                        wingvtxTrailAlphaCx, wingvtxTrailSizeCx);
                }
                ...
                if (doGVortex or doAOAVortex)
                {
                    ...
                    {
                        vortex1AlphaCx += (currentAOA - vortexAOALimit) *
                                          (1.0f / vortexAOALimit);
                    ...
                    {
                        if (largeVortex bitand
                            1) //check if bit 0 of largeVortex is set
                        {
                    ...
                    lvortex1_trail = DrawableParticleSys::PS_EmitTrail(
                        lvortex1_trail, lvortex1, lvtx1.x, lvtx1.y, lvtx1.z,
                        vortex1AlphaCx, vortex1SizeCx);
                    rvortex1_trail = DrawableParticleSys::PS_EmitTrail(
                        rvortex1_trail, rvortex1, rvtx1.x, rvtx1.y, rvtx1.z,
                        vortex1AlphaCx, vortex1SizeCx);
                    ...
                    if (vtx2.y and not(largeVortex bitand 32) and
                        not(largeVortex bitand 64) and
                        not(largeVortex bitand 128))
                    {
                        Tpoint lvtx2, rvtx2;

                        AssignACOrientation(orientation, vtx2, lvtx2, pos,
                                            false);
                        AssignACOrientation(orientation, vtx2, rvtx2, pos,
                                            true);
                        ...
                        {
                            if (largeVortex bitand
                                2) //check if bit 1 of largeVortex is set
                            {
                        ...
                        lvortex2_trail = DrawableParticleSys::PS_EmitTrail(
                            lvortex2_trail, lvortex2, lvtx2.x, lvtx2.y, lvtx2.z,
                            vortex2AlphaCx, vortex2SizeCx);
                        rvortex2_trail = DrawableParticleSys::PS_EmitTrail(
                            rvortex2_trail, rvortex2, rvtx2.x, rvtx2.y, rvtx2.z,
                            vortex2AlphaCx, vortex2SizeCx);
                    }
                    ...
                    if (((currentG > 7.0f) or
                         (currentAOA >
                          ((vortexAOALimit + vortexAOA) / 2.0f))) and
                        (vtx3.y or vtx4.y or vtx5.y))
                    {

                        //local position variables for the PS vortex effects
                        Tpoint lvtxPS1, rvtxPS1, lvtxPS2, rvtxPS2, lvtxPS3,
                            rvtxPS3;
                        ...
                        if (enablePS1 and vtx3.y)
                        {
                            ...
                            AssignACOrientation(orientation, vtx3, lvtxPS1, pos,
                                                false);
                            AssignACOrientation(orientation, vtx3, rvtxPS1, pos,
                                                true);
                            ...
                            case 1:
                                ...
                                if (largeVortex bitand
                                    4) //check if bit 2 of largeVortex is set
                                {
                            ...
                            if (vtx2.y and largeVortex bitand 32)
                            {
                                ...
                                AssignACOrientation(orientation, vtx2, lvtx2,
                                                    pos, false);
                                AssignACOrientation(orientation, vtx2, rvtx2,
                                                    pos, true);
                        ...
                        if (enablePS2 and vtx4.y)
                        {
                            ...
                            AssignACOrientation(orientation, vtx4, lvtxPS2, pos,
                                                false);
                            AssignACOrientation(orientation, vtx4, rvtxPS2, pos,
                                                true);
                            ...
                            case 1:
                                ...
                                if (largeVortex bitand
                                    8) //check if bit 3 of largeVortex is set
                                    theSFX = SFX_VORTEX_LARGE_WEAK;
                        ...
                        if (enablePS3 and vtx5.y)
                        {
                            ...
                            AssignACOrientation(orientation, vtx5, lvtxPS3, pos,
                                                false);
                            AssignACOrientation(orientation, vtx5, rvtxPS3, pos,
                                                true);
                            ...
                            case 1:
                                ...
                                if (largeVortex bitand
                                    16) //check if bit 4 of largeVortex is set
                                    theSFX = SFX_VORTEX_LARGE_WEAK;
    ...
    {
        if (PowerOutput() <= 1.0f and PowerOutput() > 0.80f and
            -ZPos() < 11000.0f)
        {
            AddEngineTrails(af->EngineTrail(), engineTrails,
                            engineTrails_trail); // smoke
        }
        else
        {
            CancelEngineTrails(engineTrails, engineTrails_trail);
    ...
    if (pctStrength > 0.50f)
    {
        if (not hasMilSmoke and
            smokeTrail
                [TRAIL_DAMAGE] /* and smokeTrail[TRAIL_DAMAGE]->InDisplayList(*/)
        {
            /* RV - I-Hawk - RV new trails call changes
            OTWDriver.AddSfxRequest(
            new SfxClass (
    ...
    if (pctStrength < 0.0f)
    {
        ...
        if (smokeTrail[TRAIL_DAMAGE])
        {
            ...
            if (orientation)
            {
                ...
                pos.x = orientation->M11 * (tp.x + offset) +
                        orientation->M12 * tp.y + orientation->M13 * tp.z +
                        XPos();
                pos.y = orientation->M21 * (tp.x + offset) +
                        orientation->M22 * tp.y + orientation->M23 * tp.z +
                        YPos();
                pos.z = orientation->M31 * (tp.x + offset) +
                        orientation->M32 * tp.y + orientation->M33 * tp.z +
                        ZPos();
                ...
                if (-ZPos() < 25000.0f)
                {
                    ...
                    damageTrailSizeCx =
                        ((15000.0f - (-ZPos())) / 15000.0f) * 0.15f + 1;
                }
                else
                {
                    damageTrailAlphaCx =
                    ...
                    damageTrailSizeCx =
                        ((15000.0f - (25000.0f)) / 15000.0f) * 0.15f + 1;
                }

                smokeTrail_trail[TRAIL_DAMAGE] =
                    DrawableParticleSys::PS_EmitTrail(
                        smokeTrail_trail[TRAIL_DAMAGE],
                        smokeTrail[TRAIL_DAMAGE], pos.x, pos.y, pos.z,
                        damageTrailAlphaCx, damageTrailSizeCx);
            }
        }
        ...
        if (smokeTrail[TRAIL_ENGINE1])
        {
            ...
            if (orientation)
            {
                ...
                smokeTrail_trail[TRAIL_ENGINE1] =
                    DrawableParticleSys::PS_EmitTrail(
                        smokeTrail_trail[TRAIL_ENGINE1],
                        smokeTrail[TRAIL_ENGINE1], pos.x, pos.y, pos.z,
                        damageTrailAlphaCx, damageTrailSizeCx);
            }
        }
    ...
    if (orientation)
    {
        ...
        pos.x = orientation->M11 * (tp.x + offset) + orientation->M12 * tp.y +
                orientation->M13 * tp.z + XPos();
        pos.y = orientation->M21 * (tp.x + offset) + orientation->M22 * tp.y +
                orientation->M23 * tp.z + YPos();
        pos.z = orientation->M31 * (tp.x + offset) + orientation->M32 * tp.y +
                orientation->M33 * tp.z + ZPos();
    ...
    smokeTrail_trail[TRAIL_DAMAGE] = DrawableParticleSys::PS_EmitTrail(
        smokeTrail_trail[TRAIL_DAMAGE], smokeTrail[TRAIL_DAMAGE], pos.x, pos.y,
        pos.z, damageTrailAlphaCx, damageTrailSizeCx);
    ...
    if (pctStrength < 0.25f)
    {
        ...
        if (orientation)
        {
            if (af->auxaeroData->nEngines >= 3 or
                (af->auxaeroData->nEngines == 2 and
                 (fabs(af->auxaeroData->engineLocation[0].y -
                       af->auxaeroData->engineLocation[1].y) > 15.0f)))
            {
                radius = 3.0f;
    ...
    if (not g_bDisableFunkyChicken and pctStrength < 0.5f and
        (rand() bitand 0xf) == 0xf)
    {
        // JB 010104
...
void AircraftClass::CheckObjectCollision(void)
{
    ...
    for (obj = targetList; obj; obj = obj->next)
    {
        ...
        if (theObject->IsWeapon() and
            ((SimWeaponClass *)theObject)->Parent() == this)
        {
            continue;
        }

        if (F4IsBadReadPtr(theObject, sizeof(SimBaseClass)) or
            not theObject->IsSim() or
            (OnGround() and not theObject->OnGround()))
        {
            continue;
        }
        ...
        if (af->IsSet(AirframeClass::OnObject) or not OnGround())
        {
            if (IsSetFlag(I_AM_A_TANKER) or theObject->IsSetFlag(I_AM_A_TANKER))
            {
                ...
                // continue;
                //}
                if (objData->range > 0.2F * theObject->drawPointer->Radius() +
                                         theObject->drawPointer->Radius())
                {
                    continue;
                }
                ...
                if (not theObject->drawPointer->GetRayHit(&org, &vec, &pos,
                                                          0.1f))
                    continue;
            }
            else
            {
                if (objData->range >
                    theObject->drawPointer->Radius() + drawPointer->Radius())
                    if (theObject->GetDomain() not_eq DOMAIN_SEA or
                        theObject->GetClass() not_eq CLASS_VEHICLE or
                        theObject->GetType() not_eq TYPE_CAPITAL_SHIP or
                        carrierInitTimer >= 30.0f)
                    {
                        continue;
                    }
                ...
                if (af->IsSet(AirframeClass::OnObject)) // JB carrier
                    org.z =
                        ZPos() +
                        5; // Make sure its detecting that we are on top // JB carrier
                else // JB carrier
                    org.z = ZPos();
        ...
        // JB carrier start
        if (IsAirplane() and theObject->GetDomain() == DOMAIN_SEA and
            theObject->GetClass() == CLASS_VEHICLE and
            theObject->GetType() == TYPE_CAPITAL_SHIP)
        {
            ...
            int numSlots =
                ((DrawableBSP *)theObject->drawPointer)->GetNumSlots();
            int numDynamics = ((DrawableBSP *)theObject->drawPointer)
                                  ->GetNumDynamicVertices();
            ...
            {
                ((DrawableBSP *)theObject->drawPointer)
                    ->GetChildOffset(takeoffSlot, &tmpPos);
            }
            else
            {
            ...
            {
                ((DrawableBSP *)theObject->drawPointer)
                    ->GetDynamicCoords(min(takeoffSlot, numDynamics - 1),
                                       &startAngle.x, &startAngle.y,
                                       &startAngle.z);
            }
            ...
            if (af->vt <= 1.0f and af->Thrust() <= 1.0f and
                (carrierInitTimer < 10.0f or theObject->YawDelta() not_eq 0.0f))
            {
                af->sigma = theObject->Yaw() + startAngle.x;
                af->ResetOrientation();
            }

            if (theObject->Yaw() >= 178.0f * DTR or
                theObject->Yaw() <= -178.0f * DTR)
            {
                startPos.x = -tmpPos.x;
                startPos.y = -tmpPos.y;
                startPos.z = tmpPos.z;
            ...
            {
                MatrixMult(
                    &((DrawableBSP *)theObject->drawPointer)->orientation,
                    &tmpPos, &startPos);
                startPos.z = tmpPos.z;
            }
            ...
            if (af->vt <= 1.0f and af->Thrust() <= 1.0f and
                carrierInitTimer < 25.0f)
            {
                if ((deltaX < abs(startPos.x) - 2.5f or
                     deltaX > abs(startPos.x) + 2.5f))
                {
                    af->x = theObject->XPos() + startPos.x;
                }

                if ((deltaY < abs(startPos.y) - 2.5f or
                     deltaY > abs(startPos.y) + 2.5f))
                {
            ...
            {
                float changeval;
                float cdof = ((DrawableBSP *)theObject->drawPointer)
                                 ->GetDOFangle(takeoffSlot + 10);
                ...
                if (af->IsSet(AirframeClass::Hook) and onCatPosition == true)
                {
                    ...
                    if (cdof < 65.0f * DTR)
                    {
                        ...
                        ((DrawableBSP *)theObject->drawPointer)
                            ->SetDOFangle(takeoffSlot + 10, cdof);
                    }

                    //RV - I-Hawk - Add Cat steam effects when trapped

                    Tpoint pos, randPos1, randPos2, randPos3, randPos4,
                        noseGear, vec;

                    noseGear.x =
                        af->GetAeroData(AeroDataSet::NosGearX) + XPos();
                    noseGear.y =
                        af->GetAeroData(AeroDataSet::NosGearY) + YPos();
                    noseGear.z =
                        af->GetAeroData(AeroDataSet::NosGearZ) - 5.0f + ZPos();
                    ...
                    //The main steam SFX
                    DrawableParticleSys::PS_AddParticleEx((SFX_CAT_STEAM + 1),
                                                          &pos, &vec);
            ...
            float gearOffset =
                min(5.0f, af->GetAeroData(AeroDataSet::NosGearZ) - 5.0f);
            ...
            noseGear.z =
                af->GetAeroData(AeroDataSet::NosGearZ) - gearOffset + ZPos();

            PSvec.x = (theObject->XDelta());
            PSvec.y = (theObject->YDelta());
            PSvec.z = 0;

            if (af->vcas > 40.0f and af->vcas < 180.0f and
                af->IsSet(AirframeClass::OnObject) and
                not(af->IsSet(AirframeClass::Hook)))
            {
                DrawableParticleSys::PS_AddParticleEx((SFX_CAT_LAUNCH + 1),
                                                      &noseGear, &PSvec);
            }
            ...
            if ((ZPos() <= minB.z * 0.96f and ZPos() > minB.z - 0.5f) or
                (ZPos() > -g_fCarrierStartTolerance and af->vcas < 10.0f))
            {
                ...
                if (carrierStartPosEngaged == 0 and
                    abs(theObject->YawDelta()) < 0.25f and
                    carrierInitTimer > 1.0f)
                {
                ...
                {
                    if (af->platform->DBrain() and
                        not af->platform->IsSetFalcFlag(FEC_INVULNERABLE))
                    {
                        af->platform->DBrain()->SetATCFlag(
                            DigitalBrain::Landed);
                        ...
                        if (af->platform not_eq SimDriver.GetPlayerEntity())
                            af->platform->DBrain()->SetWaitTimer(
                                SimLibElapsedTime + 1 * CampaignMinutes);
                    }
                }
                ...
                noseDir.x = af->platform->platformAngles.costhe *
                            af->platform->platformAngles.cospsi;
                noseDir.y = af->platform->platformAngles.costhe *
                            af->platform->platformAngles.sinpsi;
                noseDir.z = -af->platform->platformAngles.sinthe;
                tmp =
                    (float)sqrt(noseDir.x * noseDir.x + noseDir.y * noseDir.y +
                                noseDir.z * noseDir.z);
                noseDir.x /= tmp;
                noseDir.y /= tmp;
                noseDir.z /= tmp;

                noseAngle = af->gndNormal.x * noseDir.x +
                            af->gndNormal.y * noseDir.y +
                            af->gndNormal.z * noseDir.z;
                impactAngle = af->gndNormal.x * velocity.x +
                              af->gndNormal.y * velocity.y +
                              af->gndNormal.z * velocity.z;
                ...
                if (not af->IsSet(AirframeClass::InAir) or
                    af->platform->LandingCheck(noseAngle, impactAngle,
                                               COVERAGE_OBJECT))
                {
        ...
        if (not af->IsSet(AirframeClass::OnObject) and OnGround() and
            af->vcas <= 50.0f and gCommsMgr and gCommsMgr->Online())
        {
            continue;
        }
        ...
        if (theObject->GetType() == TYPE_BOMB and
            (theObject->GetSType() == STYPE_CHAFF or
             theObject->GetSType() == STYPE_FLARE1) and
            (theObject->GetSPType() == SPTYPE_CHAFF1 or
             theObject->GetSPType() == SPTYPE_CHAFF1 + 1))
            continue;
        ...
        if (theObject->IsAirplane())
            message->dataBlock.fPilotID =
                ((SimMoverClass *)theObject)->pilotSlot;
        else
        ...
        ShiAssert(GetCampaignObject()) message->dataBlock.dCampID = GetCampID();
        ...
        message->dataBlock.damageStrength =
            min(1000.0F, relVel * theObject->Mass() * 0.0001F +
                             relVel * relVel * theObject->Mass() * 0.000002F);
        ...
        ShiAssert(GetCampaignObject()) message->dataBlock.fCampID = GetCampID();
        ...
        message->dataBlock.damageRandomFact = 1.0f;
        message->dataBlock.damageStrength =
            min(1000.0F, relVel * Mass() * 0.0001F +
                             relVel * relVel * Mass() * 0.000002F);
...
void AircraftClass::AddFault(int failures, unsigned int failuresPossible, int,
                             int)
{
    int i;
                         ...
                         Tpoint &position, Tpoint &worldPosition, bool right)
{
    ...
    if (right)
    {
        position.x = orientation->M11 * origin.x + orientation->M12 * origin.y +
                     orientation->M13 * origin.z + worldPosition.x;
        position.y = orientation->M21 * origin.x + orientation->M22 * origin.y +
                     orientation->M23 * origin.z + worldPosition.y;
        position.z = orientation->M31 * origin.x + orientation->M32 * origin.y +
                     orientation->M33 * origin.z + worldPosition.z;
    }
    else
    {
        position.x = orientation->M11 * origin.x +
                     orientation->M12 * -origin.y +
                     orientation->M13 * origin.z + worldPosition.x;
        position.y = orientation->M21 * origin.x +
                     orientation->M22 * -origin.y +
                     orientation->M23 * origin.z + worldPosition.y;
        position.z = orientation->M31 * origin.x +
                     orientation->M32 * -origin.y +
                     orientation->M33 * origin.z + worldPosition.z;
    }
}

...
extern int
    g_nTaxiLaunchTime; //RAS - 14Oct04 - need Boosters value to do quick preflight earlier if
//user decides to start taxi earlier than 3 min (default quickpreflight)

#define MoveAlong                                                              \
    3.0f // 05JAN04 - FRB - was 5 secs to travel between taxi points
...
#define AI_MESSAGE(slot, str)                                                  \
    ...
    ofstream logFile("c:/temp/log.txt");
}
#endif
...
void DigitalBrain::ResetTaxiState(void)
{
    ...
    if ((atcstatus == tTakeRunway or atcstatus == tTakeoff) and
        (PtDataTable[curTaxiPoint].type == TakeoffPt or
         PtDataTable[curTaxiPoint].type == RunwayPt))
    {
        rwLocalIndex = Airbase->brain->IsOnRunway(self);

        takeoffpt = Airbase->brain->FindTakeoffPt(
            (Flight)self->GetCampaignObject(), self->vehicleInUnit, rwIndex,
            &x1, &y1);
        runwaypt = Airbase->brain->FindRunwayPt(
            (Flight)self->GetCampaignObject(), self->vehicleInUnit, rwIndex,
            &x2, &y2);

        float cosAngle =
            self->platformAngles.sinsig *
                PtHeaderDataTable[rwIndex].sinHeading +
        ...
        dx = runwayStats[PtHeaderDataTable[rwIndex].runwayNum].centerX -
             self->XPos();
        dy = runwayStats[PtHeaderDataTable[rwIndex].runwayNum].centerY -
             self->YPos();

        relx = PtHeaderDataTable[rwIndex].cosHeading * dx +
               PtHeaderDataTable[rwIndex].sinHeading * dy;

        if (cosAngle > 0.99619F and
            PtHeaderDataTable[rwIndex].runwayNum ==
                PtHeaderDataTable[rwLocalIndex].runwayNum and
            runwayStats[PtHeaderDataTable[rwIndex].runwayNum].halfheight +
                    relx >
                3000.0F)
        {
            SetTrackPoint(x2, y2);
        ...
        {
            SetTrackPoint(x1 - relx * PtHeaderDataTable[rwIndex].cosHeading,
    ...
    {
        if (PtDataTable[curTaxiPoint].type == TakeoffPt or
            PtDataTable[curTaxiPoint].type == RunwayPt)
        {
            float tx, ty;
            takeoffpt = Airbase->brain->FindTakeoffPt(
        ...
        else
        {
            ...
            dx = x1 - af->x;
            dy = y1 - af->y;
            relx = self->platformAngles.cospsi * dx +
                   self->platformAngles.sinpsi * dy;
...
{
    float curHeading = 0.0F, newHeading = 0.0F, cosAngle = 1.0F,
          deltaHdg = 0.0F;
    float baseX = 0.0F, baseY = 0.0F, finalX = 0.0F, finalY = 0.0F, dx = 0.0F,
          dy = 0.0F, vt = 0.0F;
    ObjectiveClass *Airbase = (ObjectiveClass *)vuDatabase->Find(airbase);
    ...
    if (Airbase and Airbase->brain)
    {
        ...
        case lFirstLeg:
            ...
            if (cosAngle < 0.0F)
            {
                Airbase->brain->FindBasePt(self, rwIndex, finalX, finalY,
                                           &baseX, &baseY);
                dx = trackX - baseX;
                dy = trackY - baseY;
            }
...
void DigitalBrain::Land(void)
{
    ...
    unsigned int mini, maxi; // CampaignTime (32-bit)
    float baseX, baseY, finalX, finalY, finalZ, x, y, z, dx, dy, dist, speed,
        minSpeed, relx, rely, cosHdg, sinHdg;
    mlTrig Trig;
    ...
    if (self->IsSetFlag(ON_GROUND) and (af->Fuel() <= 0.0F) and
        (self->GetVt() < 5.0F))
    {
    ...
    if (rwIndex > 0)
    {
        ...
        abObj->brain->CalculateMinMaxTime(self, rwIndex, atcstatus, &mini,
                                          &maxi, cosAngle);
    }
    else
    {
    ...
    case noATC:
        ...
        if (rwIndex == 0)
        {
            ...
            rwIndex = (abObj == NULL) ?
                          0 :
                          abObj->brain->FindBestLandingRunway(self, TRUE);
        ...
        TrackPointLanding(af->CalcTASfromCAS(af->MinVcas() * 1.2F) *
                          KNOTS_TO_FTPSEC);
        dx = self->XPos() - abObj->XPos();
        dy = self->YPos() - abObj->YPos();

        //me123
        if (curMode not_eq LandingMode and curMode not_eq TakeoffMode and
            curMode not_eq WaypointMode and curMode not_eq RTBMode)
        {
            break;
        }
    ...
    case lIngressing:
        ...
        {
            TrackPointLanding(af->CalcTASfromCAS(af->CornerVcas()) *
                              KNOTS_TO_FTPSEC);
        }
    ...
    case lTakingPosition:
        ...
        if (dx * dx + dy * dy < TOWER_RANGE * NM_TO_FT * NM_TO_FT * 0.5F and
        ...
        {
            {
                ...
                while (component and
                       component->vehicleInUnit not_eq self->vehicleInUnit)
                {
                    leader = component;
    ...
    case lReqEmerClearance:
        ...
        if (SimLibElapsedTime > waittimer)
        {
            ...
            if (dx * dx + dy * dy <
                (TOWER_RANGE + 100) * NM_TO_FT * NM_TO_FT * 0.95F)
            {
                waittimer = SimLibElapsedTime + LAND_TIME_DELTA;
                SendATCMsg(lReqClearance);
            }
    ...
    case lAborted:
        ...
        TrackPoint(0.0F,
                   af->CalcTASfromCAS(af->MinVcas() * 1.2F) * KNOTS_TO_FTPSEC);
        af->gearHandle = -1.0F;
        break;
    ...
    case lHolding:
        ...
        if (rwtime < SimLibElapsedTime + maxi - CampaignSeconds * 5)
        {
            ...
            if (cosAngle < 0.0F)
            {
                abObj->brain->FindBasePt(self, rwIndex, finalX, finalY, &baseX,
                                         &baseY);
                float tx, ty, tz;
                atcstatus = abObj->brain->FindFirstLegPt(
                    self, rwIndex, rwtime, baseX, baseY, TRUE, &tx, &ty);
                tz = abObj->brain->GetAltitude(self, atcstatus);
                SetTrackPoint(tx, ty, tz);
            ...
            {
                trackZ =
                    gAvoidZ + minZ - (self->ZPos() - gAvoidZ - minZ) * 2.0f;
            }
    ...
    case lFirstLeg:
        ...
        if (fabs(relx) < turnDist and fabs(rely) < turnDist * 3.0F)
        {
            ...
            if (cosAngle < 0.0F)
            {
                ...
                abObj->brain->FindBasePt(self, rwIndex, finalX, finalY, &baseX,
                                         &baseY);
                SetTrackPoint(baseX, baseY,
                              abObj->brain->GetAltitude(self, atcstatus));
                SendATCMsg(atcstatus);
                CalculateNextTurnDistance();
            }
            else
    ...
    case lEmergencyToBase:
        ...
        {
            deltaTime = (rwtime - SimLibElapsedTime - FINAL_TIME - BASE_TIME) /
                        (float)CampaignSeconds;
            speed = (float)sqrt(dx * dx + dy * dy) / deltaTime;
            speed =
                min(af->CalcTASfromCAS(af->MaxVcas() * 0.8F) * KNOTS_TO_FTPSEC,
                    max(speed, af->CalcTASfromCAS(af->MinVcas() * 0.8F) *
                                   KNOTS_TO_FTPSEC));
        }
        else
        {
    ...
    case lEmergencyToFinal:
        ...
        {
            deltaTime = (rwtime - SimLibElapsedTime - FINAL_TIME) /
                        (float)CampaignSeconds;
            speed = (float)sqrt(dx * dx + dy * dy) / deltaTime;
            speed =
                min(af->CalcTASfromCAS(af->MaxVcas() * 0.8F) * KNOTS_TO_FTPSEC,
    ...
    case lOnFinal:
        ...
        if (cosAngle > 0.0F and af->groundZ - self->ZPos() > 50.0F)
        {
            ...
            deltaTime =
                ((float)rwtime - (float)SimLibElapsedTime) / CampaignSeconds;
            testDist =
                (minSpeed * 0.2F / (FINAL_TIME / CampaignSeconds) * deltaTime +
                 minSpeed * 0.6F) *
                deltaTime;
            desiredSpeed =
                (minSpeed * 0.4F / (FINAL_TIME / CampaignSeconds) * deltaTime +
                 minSpeed * 0.6F);

            if (dist > minSpeed * 19.5F)
            {
                desiredSpeed +=
                    (dist - testDist) / (testDist * 0.8F) * desiredSpeed;
            }
            ...
            desiredSpeed =
                max(min(minSpeed * 1.2F, desiredSpeed), minSpeed * 0.6F);
    ...
    case lLanded:
        ...
        {
            heading =
                (float)atan2(trackX - self->XPos(), trackY - self->YPos());
        ...
        if (self->GetVt() <
            af->MinVcas() *
                0.5F) // THW 2003-11-23 more clean up (this one a bit later)
        {
            if (af->tefPos > 0)
                af->TEFClose();
        ...
        {
            SimpleGroundTrack(min(
                60.0F * KNOTS_TO_FTPSEC,
                af->MinVcas() *
                    0.4F)); //THW 2003-11-23 No hurry...no need to re-accelerate
        }
        else if (dx * dx + dy * dy > 500.0F * 500.0F) //THW 2003-11-23 Slow down
        {
            SimpleGroundTrack(
                min(50.0F * KNOTS_TO_FTPSEC, af->MinVcas() * 0.3F));
        }
        ...
        {
            char label[96];
            sprintf(label, "Speed: %d kts, PointDist: %d ft",
                    FloatToInt32(self->GetVt() / KNOTS_TO_FTPSEC),
                    FloatToInt32(sqrt(dx * dx + dy * dy)));
    ...
    case lTaxiOff:
        ...
        if (AtFinalTaxiPoint())
        {
            ...
            else
            {
                desiredSpeed = 20.0F * KNOTS_TO_FTPSEC *
                               (float)sqrt(dx * dx + dy * dy) / TAXI_CHECK_DIST;
            }

            desiredSpeed = min(20.0F * KNOTS_TO_FTPSEC, desiredSpeed);

            if (waittimer < SimLibElapsedTime or
                (desiredSpeed == 0 and self->GetVt() < 0.1f))
            {
                ...
                else if (af->GetNumberEngines() == 2 and
                         not af->IsEngineFlag(AirframeClass::EngineStopped2))
                {
                ...
                else if (af->rpm < 0.05f and
                         self->MainPower() not_eq AircraftClass::MainPowerOff)
                {
                    self->DecMainPower();
                }
                else if (self not_eq SimDriver.GetPlayerEntity() and
        ...
        if (inTheWay and inTheWay->GetVt() < 10.0F)
        {
            ...
            default:
                ...
                if ((((AircraftClass *)inTheWay)->DBrain()) and
                    ((PtDataTable[((AircraftClass *)inTheWay)
                                      ->DBrain()
                                      ->GetTaxiPoint()]
                              .type == LargeParkPt or
                      (PtDataTable[((AircraftClass *)inTheWay)
                ...
                {
                    OffsetTrackPoint(TAXI_CHECK_DIST,
                                     offRight); // JPO from offRight
                }
...
int DigitalBrain::BestParkSpot(
    ObjectiveClass
        *Airbase) // FRB - Landing-->Park last-to-first or first-to-last???
{
    float x, y;
    int pktype = af->GetParkType();
    int npt, pt = curTaxiPoint;
#if 1 // 17JAN04 - FRB - Find the *closest* available parking spot.
    pt = GetNextParkTypePt(curTaxiPoint,
                           pktype); // find first available parking spot
    npt = pt;
    ...
    while ((npt = GetNextParkTypePt(pt, pktype)) not_eq
           0) // find last possible parking spot
        pt = npt;

    if (PtDataTable[pt].type not_eq
        pktype) // just in case we found none  // FRB - isn't pt == 0 here???
        return 0;
    ...
    while (pt > curTaxiPoint)
    {
        ...
        pt = GetPrevParkTypePt(
            pt,
            pktype); // ok try another  // FRB - shouldn't it be GetNextParkTypePt
    }
...
void DigitalBrain::FindParkingSpot(
    ObjectiveClass *
        Airbase) // FRB - Landing-->Park (repeated calls - checks next pt for available parking spot)
{
#if 1
    ...
    while (PtDataTable[pt].flags not_eq PT_LAST)
    {
        ...
        else if (PtDataTable[pt].type ==
                     SmallParkPt or // already used (not bestpt), skip
...
void DigitalBrain::SetDebugLabel(ObjectiveClass *Airbase)
{
    ...
    else
    {
        ...
        if (info)
        {
            ...
            sprintf(tmpStr, "%d %d %d %d %3.1f %3.1f %3.1f", pos, curTaxiPoint,
                    atcstatus, status, desiredSpeed, rwdelta / 1000.0F,
                    SimLibElapsedTime / 60000.0F);
        }
        else
            sprintf(tmpStr, "%d %d %3.1f %3.1f %3.1f", curTaxiPoint, atcstatus,
                    desiredSpeed, rwdelta / 1000.0F,
                    SimLibElapsedTime / 60000.0F);
...
void DigitalBrain::TakeOff()
{
    ...
    AircraftClass *flight_leader =
        NULL; //RAS - 12Nov04 - used to check for human movement in RAMP start


    flight_leader =
        (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(0); //RAS
    ...
    if (self == SimDriver.GetPlayerEntity() and IsSetATC(StopPlane) and
    ...
    if (not IsSetATC(DonePreflight) and curTaxiPoint)
    {
        ...
        if (rwtime > 0) // value given by ATC
            ...
            t2t = self->curWaypoint
                      ->GetWPDepartureTime(); // else original scheduled time

        // Cobra - Start with canopy open if on parking spot
        if ((PlayerOptions.GetStartFlag() not_eq
             PlayerOptionsClass::START_RUNWAY) and
            ((af->GetParkType() not_eq LargeParkPt)))
        {
            ...
            if (TheTimeOfDay.GetLightLevel() < 0.65f)
            {
                self->SetSwitch(COMP_3DPIT_INTERIOR_LIGHTS,
                                TRUE); // Cobra - Light up cockpit
                self->SetAcStatusBits(AircraftClass::ACSTATUS_PITLIGHT);
            }
            else
            {
                self->SetSwitch(COMP_3DPIT_INTERIOR_LIGHTS,
                                FALSE); // Cobra - Light up cockpit
                self->ClearAcStatusBits(AircraftClass::ACSTATUS_PITLIGHT);
            }
        }
        ...
        if (SimLibElapsedTime >
            t2t - max(g_nTaxiLaunchTime * CampaignMinutes, 3 * CampaignMinutes))
        {
            // emergency pre-flight
            QuickPreFlight();
        }
        else if (SimLibElapsedTime <
                 t2t - PlayerOptionsClass::RAMP_MINUTES * CampaignMinutes)
        {
            return; // not time for startup yet
        }
        else if (PtDataTable[curTaxiPoint].flags == PT_LAST or
                 ...
                 PtDataTable[curTaxiPoint].type == LargeParkPt)
        {
            ...
            if (isWing and flight_leader and
                flight_leader->AutopilotType() not_eq
                    AircraftClass::CombatAP and
                flight_leader->DBrain()->curTaxiPoint not_eq
                    flight_leader->spawnpoint)
            {
                QuickPreFlight();
        ...
        if (isWing and flight_leader and flight_leader->IsPlayer())
        {
            ...
            radioMessage->dataBlock.edata[2] = ConvertWingNumberToCallNumber(
        ...
        if (flight)
        {
            ...
            if (w)
            {
                ...
                if (curTaxiPoint)
                {
                    ...
                    int npt = GetPrevTaxiPt(
                        curTaxiPoint); // this is next place to go to
                    float x, y;
                    TranslatePointData(Airbase, npt, &x, &y);
                    self->af->initialPsi = self->af->psi = self->af->sigma =
                        (float)atan2((y - self->af->y), (x - self->af->x));
    ...
    case noATC:
        ...
        case TakeoffPt:
        {
            ...
            {
                SetTaxiPoint(Airbase->brain->FindTakeoffPt(
                    (Flight)self->GetCampaignObject(), self->vehicleInUnit,
                    rwIndex, &tx, &ty));
            }
            else
            {
        ...
        case RunwayPt:
        {
            ...
            {
                SetTaxiPoint(Airbase->brain->FindRunwayPt(
                    (Flight)self->GetCampaignObject(), self->vehicleInUnit,
                    rwIndex, &tx, &ty));
            }
            else
        ...
        default:
        {
            ...
            waittimer =
                SimLibElapsedTime +
                (VU_TIME)(HurryUp * CampaignSeconds); // FRB - speed up moving
    ...
    case tWait:
        ...
        if ((distSq < TAXI_CHECK_DIST * TAXI_CHECK_DIST) or
    ...
    case tHoldShort:
        ...
        if (rwtime < (MoveAlong * CampaignSeconds + SimLibElapsedTime) and
            waittimer < SimLibElapsedTime)
        {
            SendATCMsg(atcstatus);
    ...
    case tTakeRunway:
        ...
        {
            if (isWing and self->af->IsSet(AirframeClass::OverRunway) and
                not WingmanTakeRunway(Airbase))
            {
                float tx, ty;
                SetTaxiPoint(Airbase->brain->FindTakeoffPt(
                    (Flight)self->GetCampaignObject(), self->vehicleInUnit,
            ...
            {
                float cosAngle = self->platformAngles.sinsig *
                                     PtHeaderDataTable[rwIndex].sinHeading +
                                 self->platformAngles.cossig *
                                     PtHeaderDataTable[rwIndex].cosHeading;
    ...
    case tFlyOut:
        ...
        if (af->z - af->groundZ > -200.0F or
            (fabs(xft) < 200.0F and fabs(yft) < 200.0F))
        {
            break;
        }
        ...
        if (not isWing and elements > 1) // Code for the flightlead
        {
            ...
            AircraftClass *elemleader =
                (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(
                    elements == 2 ? 1 : 2);

            if (elemleader) // is #3 in a 4- and 3-ship flight, #2 in a 2-ship flight
            {
                airbase = self->LandingAirbase(); // JPO - now we set to go home
                ObjectiveClass *Airbase =
                    (ObjectiveClass *)vuDatabase->Find(airbase);

                if (not Airbase or
                    elemleader->af->z - elemleader->af->groundZ <
                        -50.0F) // #3 has taken off -> lead continue to next WP
                {
                ...
                else
                {
                    ...
                    acHeading =
                        self->Yaw(); // fix, use aircrafts real heading instead of runway heading
                    ...
                    if (tmpWaypoint)
                    {
                        ...
                        ShiAssert(
                            not F4IsBadReadPtr(flight, sizeof(FlightClass)));
                        float factor = 0.0F;

                        if (flight)
                        {
                            wingman =
                                (AircraftClass *)flight->GetComponentNumber(
                                    1); // my wingy = 1 in each case
                            ShiAssert(not F4IsBadReadPtr(
                                wingman, sizeof(AircraftClass)));

                            if (wingman and wingman->af and
                                (wingman->af->z - wingman->af->groundZ <
                                 -200.0F)) // My wingman has flown out
                                factor = 45.0F * DTR;

                            ShiAssert(Airbase->brain);

                            if (Airbase->brain and
                                Airbase->brain->UseSectionTakeoff(
                                    flight,
                                    rwIndex)) // If our wingman took off with us, stay on a 90� leg
                                factor = 0.0F;
                        }
                        ...
                        SimpleTrack(
                            SimpleTrackSpd,
                            (af->MinVcas() *
                             1.2f)); // fly as slow as possible ~ 178 kts
    ...
    case tEmerStop:
        ...
        if (fabs(trackX - af->x) > TAXI_CHECK_DIST or
            fabs(trackY - af->y) > TAXI_CHECK_DIST)
        {
            inTheWay = CheckTaxiTrackPoint();
...
{
    AircraftClass *leader =
        (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(
            self->vehicleInUnit - 1);

    return WingmanTakeRunway(Airbase, (AircraftClass *)flightLead, leader);
}

int DigitalBrain::WingmanTakeRunway(ObjectiveClass *Airbase,
                                    AircraftClass *FlightLead,
                                    AircraftClass *leader)
{
    int pt;
    float tempX, tempY;
    ...
    case 1:
        ...
        if (not Airbase->brain->IsOnRunway(tempX, tempY))
            ...
            return TRUE;
        else if (Airbase->brain->IsOnRunway(FlightLead) and
                 Airbase->brain->UseSectionTakeoff(
                     (Flight)self->GetCampaignObject(), rwIndex))
            return TRUE;
        //RAS-28Oct04-Speed up #2 taking Rwy
        else if (Airbase->brain->IsOnRunway(FlightLead) and
                 not Airbase->brain->UseSectionTakeoff(
                     (Flight)self->GetCampaignObject(), rwIndex) and
    ...
    case 2: // Element Leader
        ...
        if (FlightLead and
            not FlightLead
                    ->OnGround()) // Cobra - if Flight leader is in the air
        {
            if ((FlightLead->GetVt() > 40 * KNOTS_TO_FTPSEC) and leader and
        ...
        else if (leader and leader->OnGround() and
                 leader->GetVt() > 40.0 * KNOTS_TO_FTPSEC)
            return TRUE;
    ...
    case 3: // Element Wingman
        ...
        if (not Airbase->brain->IsOnRunway(tempX, tempY))
            return TRUE;
        else if (leader and Airbase->brain->IsOnRunway(leader) and
                 Airbase->brain->UseSectionTakeoff(
                     (Flight)self->GetCampaignObject(), rwIndex))
            return TRUE;
        ...
        {
            if (FlightLead->GetVt() > 50 * KNOTS_TO_FTPSEC and leader and
                leader->GetVt() > 40.0 * KNOTS_TO_FTPSEC)
                return TRUE;
        }

        FlightLead =
            (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(1);
        ...
        if (FlightLead and not FlightLead->OnGround())
        {
            ...
            return TRUE;
        else if (Airbase->brain->IsOnRunway(leader) and
                 Airbase->brain->UseSectionTakeoff(
                     (Flight)self->GetCampaignObject(), rwIndex))
            return TRUE;
...
void DigitalBrain::DealWithBlocker(SimBaseClass *inTheWay,
                                   ObjectiveClass *Airbase)
{
    ...
    if (inTheWay->GetCampaignObject() == self->GetCampaignObject() and
        self->vehicleInUnit > ((AircraftClass *)inTheWay)->vehicleInUnit)
    {
    ...
    {
        extraWait =
            FalconLocalGame->rules.AiPatience; // FRB - not *that* patient

        if (rwtime > SimLibElapsedTime)
            extraWait += (rwtime - SimLibElapsedTime) /
                         20; // FRB - Could be a long extra wait (was 20)
    }

    if (not inTheWay2 and
        (not inTheWay->IsAirplane() or
         ((AircraftClass *)inTheWay)->DBrain()->RwTime() > rwtime))
    {
    ...
    else if ((isWing and (not inTheWay2 or
                          inTheWay2->GetCampaignObject() not_eq
                              self->GetCampaignObject() or
                          self->vehicleInUnit <
                              ((AircraftClass *)inTheWay2)->vehicleInUnit)) or
             (inTheWay->GetVt() < 5.0F and
              SimLibElapsedTime > waittimer + extraWait) or
             (inTheWay->IsAirplane() and
              ((AircraftClass *)inTheWay)->DBrain()->RwTime() > rwtime))
    {
...
void DigitalBrain::ChooseNextPoint(
    ObjectiveClass *Airbase) // to Runway  Takeoff() bitand ResetTaxiState()
{
    int pt = 0;
    ...
    flight_leader =
        (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(0);
    ...
    if (flight_leader and flight_leader->IsPlayer() and
        flight_leader->AutopilotType() not_eq AircraftClass::CombatAP)
    {
        HP_Is_Leader = TRUE;
        ...
        if (HP_Moving)
        {
            ...
            PtDataTable[flight_leader->spawnpoint].flags and_eq
                compl PT_OCCUPIED;
            ((AircraftClass *)flight_leader)
                ->DBrain()
                ->SetTaxiPoint(GetFirstPt(rwIndex));
    ...
    if (flight_leader and
        ((flight_leader->IsPlayer() and
          (flight_leader->AutopilotType() == AircraftClass::CombatAP)) or
         not flight_leader->IsPlayer()))
    {
        if (SimLibElapsedTime < waittimer and not IsSetATC(PermitRunway) and
            not IsSetATC(PermitTakeRunway))
        {
            return;
        }
    }
    ...
    if (isWing)
    {
        leader = (AircraftClass *)self->GetCampaignObject()->GetComponentNumber(
            self->vehicleInUnit - 1);

        if (HP_Is_Leader and HP_Moving and leader and
            (leader == flight_leader or leader->IsPlayer()))
        {
            // Cobra
            minPoint = GetFirstPt(rwIndex);
        }
        else if (leader and leader->IsPlayer() and HP_Moving and
                 leader->AutopilotType() not_eq AircraftClass::CombatAP)
        {
            // Cobra
            minPoint = GetFirstPt(rwIndex);
        }
        else if (leader and leader->OnGround() and
                 leader->DBrain()->ATCStatus() not_eq tTaxiBack)
        {
    ...
    case TaxiPt:
        ...
        if (isWing and leader and
            (PtDataTable[curTaxiPoint].type == SmallParkPt or
             PtDataTable[curTaxiPoint].type == LargeParkPt) and
            ((PtDataTable[leader->DBrain()->curTaxiPoint].type == SmallParkPt or
              PtDataTable[leader->DBrain()->curTaxiPoint].type ==
                  LargeParkPt) or
        ...
        PtDataTable[self->spawnpoint].flags and_eq
            compl PT_OCCUPIED; // Cobra - Make HP's parking spot available

        //just taxi along
        pt = GetPrevTaxiPt(
            curTaxiPoint); // 17JAN04 - FRB - Get taxi pt (prev pt may be parking pt)
    ...
    case TakeRunwayPt:
        ...
        if (isWing)
        {
            if (GetPrevPtLoop(curTaxiPoint) == minPoint and
                not IsSetATC(PermitRunway))
            {
                return;
            }
            ...
            else if (not Airbase->brain->IsOnRunway(
                         GetPrevPtLoop(GetPrevPtLoop(curTaxiPoint))))
            {
                ...
                SetTrackPoint(tx, ty);
                CalculateTaxiSpeed(
                    HurryUp); // FRB - he is already at least 2 taxipts behind
        ...
        else if (IsSetATC(PermitRunway) and
        ...
        else if (IsSetATC(PermitTakeRunway) and
                 not self->IsSetFalcFlag(FEC_HOLDSHORT))
        {
            pt = GetPrevPtLoop(curTaxiPoint);
            float tempX, tempY;
        ...
        else if (PtDataTable[curTaxiPoint].type == TakeRunwayPt and
                 not self->IsSetFalcFlag(FEC_HOLDSHORT))
        {
        ...
        {
            if (atcstatus not_eq tTakeRunway and
                not self->IsSetFalcFlag(FEC_HOLDSHORT))
            {
    ...
    case TakeoffPt:
                 ...
                 not self->IsSetFalcFlag(FEC_HOLDSHORT))
        {
            ...
            waittimer =
                CalcWaitTime(Airbase->brain); // FRB - do we have to wait?
            //waittimer = 0;
        }
        else if (self->af->IsSet(AirframeClass::OverRunway))
        {
        ...
        else if (PtDataTable[curTaxiPoint].type not_eq TakeRunwayPt and
    ...
    {
        if (isWing and
            not WingmanTakeRunway(Airbase, (AircraftClass *)flightLead,
                                  leader) and
...
void DigitalBrain::SendATCMsg(AtcStatusEnum msg)
{
    ...
    if (g_bMPFix)
    {
        ATCMessage = new FalconATCMessage(
            airbase, (VuTargetEntity *)vuDatabase->Find(
                         vuLocalSessionEntity->Game()->OwnerId()));
    }
    else
    {
...
float DigitalBrain::CalculateTaxiSpeed(
    float time) // to T/O  Need to fix TaxiPt intervals in AB PDs
{
    ObjectiveClass *Airbase = (ObjectiveClass *)vuDatabase->Find(airbase);
    ...
    point = GetNextTaxiPt(
        curTaxiPoint); // 07JAN04 - FRB - Get *taxi* point we are at now
    ...
    desiredSpeed = max(
        5.0F * KNOTS_TO_FTPSEC,
        min(desiredSpeed,
            24.0F *
                KNOTS_TO_FTPSEC)); //RAS - 10Oct04 - changed from 30 to 24 max based on Booster's input
...
int DigitalBrain::FindDesiredTaxiPoint(ulong takeoffTime)
{
    ...
    {
        time_til_takeoff =
            (takeoffTime - Camp_GetCurrentTime()) / (TAKEOFF_TIME_DELTA);
    }
    ...
    if ((af->GetParkType() == SmallParkPt) and
        (tp = GetNextParkTypePt(tp, af->GetParkType())))
    {
        PtDataTable[tp].flags or_eq
            PT_OCCUPIED; // 02JAN04 - FRB - Block use of first parking spot
    }
    ...
    while (tp)
    {
        ...
        if ((PtDataTable[tp].type == SmallParkPt) or
            (PtDataTable[tp].type == LargeParkPt))
        {
    ...
    if (parkPt >= 0)
    {
        tp = parkPt;
        PtDataTable[tp].flags or_eq
            PT_OCCUPIED; // 02JAN04 - FRB - Reserve parking spot
...
int DigitalBrain::CalcWaitTime(ATCBrain *Atc)
{
    ...
    case tTakeoff:
        time = SimLibElapsedTime +
               static_cast<VU_TIME>(MoveAlong *
                                    CampaignSeconds); // FRB - 5 ==> MoveAlong
        break;
    ...
    case tReqTakeoff:
        ...
        time = SimLibElapsedTime +
               static_cast<VU_TIME>(MoveAlong * CampaignSeconds); // FRB - 2
        break;
    ...
    default:
        ...
        {
            time =
                ((rwtime - (count * (VU_TIME)(MoveAlong * CampaignSeconds) +
                            SimLibElapsedTime)) +
                 SimLibElapsedTime); // FRB - (count * TAKEOFF_TIME_DELTA) is delta time
            time -=
                (VU_TIME)(g_fTaxiEarly *
                          CampaignSeconds); //RAS - 10Oct04 - Added hold short delay to start taxi sooner

            if (time < SimLibElapsedTime)
                time = SimLibElapsedTime;
        }

        else if (PtDataTable[curTaxiPoint].type <=
                 TakeoffPt) //        rwtime is Sim clock time
...
int DigitalBrain::ReadyToGo(void)
{
    ...
    if ((not isWing or self->vehicleInUnit == 2) and
        rwtime + WINGMAN_WAIT_TIME <
            SimLibElapsedTime and // FRB - check WINGMAN_WAIT_TIME value (is 30 secs)
        waittimer <= SimLibElapsedTime)
    {
        retval = TRUE;
    }
        ...
        if (Airbase->brain->UseSectionTakeoff(flight, rwIndex))
        {
            ...
            if (wingman and Airbase->brain)
            {
                ...
                if (not wingman->OnGround())
                    retval = TRUE;
                else if (wingman->GetVt() > 40.0F * KNOTS_TO_FTPSEC and
                         (runway == rwIndex or
                          Airbase->brain->GetOppositeRunway(runway) == rwIndex))
                    retval = TRUE;

                if (isWing == 0 or self->vehicleInUnit == 2)
                {
                    if (wingman->GetVt() < 2.0F and
                        (runway == rwIndex or
                         Airbase->brain->GetOppositeRunway(runway) == rwIndex))
                        retval = TRUE;
                }
...
BOOL DigitalBrain::CheckTaxiCollision(void)
{
    ...
    while (campBase)
    {
        ...
        while (testObject)
        {
            ...
            // Not on ground
            // no drawpointer
            if (not testObject->OnGround() or testObject == self or
...
BOOL DigitalBrain::SimpleGroundTrack(float speed)
{
    ...
    if (speed >
        0.0f) //RAS - 14Oct04 - Keep AI from rolling if they aren't supposed to, if Player can't move, then change this
        af->ClearFlag(
            AirframeClass::
                WheelBrakes); //RAS - may need to add this line to playerEntity section below
    ...
    if (self == SimDriver.GetPlayerEntity() and IsSetATC(StopPlane))
    {
        ...
        if (rx < 10.0F)
        {
            ...
            int taxiPoint = GetPrevTaxiPt(
                curTaxiPoint); // 03JAN04 - FRB - Skip Parking spots
            ...
            if (gameCompressionRatio)
                ...
                rStick = SimpleTrackAzimuth(
                    rx, ry, self->GetVt()); ///gameCompressionRatio;
            else
                rStick = 0.0f;
    ...
    else
    {
        ...
        if (dist <
            TAXI_CHECK_DIST *
                TAXI_CHECK_DIST) // * 4.0F) // Cobra - 120' may be not close enough
        {
    ...
    // loop thru all sim objects
    {
        ...
        while (testObject)
        {
            ...
            if ((BigBoy and (fabs(az) > 10.f * DTR)) or (fabs(az) > MAX_AZ))
            {
                testObject = (SimBaseClass *)unitWalker.GetNext();
                continue;
            }
            ...
            */
            if (rx > 0.0F and fabs(ry) > testRad and range < 0.0F and
                ...
                self->vehicleInUnit >
                    ((AircraftClass *)testObject)->vehicleInUnit)
            {
                xft = trackX - af->x;
                yft = trackY - af->y;
                // get relative position and az
            ...
            if (fabs(az) < 80.0F * DTR and rx > 5.0f and rx < testRad * 1.5F and
                fabs(ry) < testRad * 1.25F)
            {
    ...
    {
        throtl = SimpleGroundTrackSpeed(
            speed * 0.75f); // speed = desired speed (ft/sec)
    }

    return FALSE;
}

void DigitalBrain::UpdateTaxipoint(
    void) // Called by Airframe.cpp, AircraftInputs.cpp
{
    ...
    if ((not self->OnGround()) or
        not self->IsPlayer() and (atcstatus == tTakeRunway))
        return;
    ...
    if (closest not_eq curTaxiPoint)
    {
        ...
        if (closest == GetNextTaxiPt(curTaxiPoint) and
            self->AutopilotType() == AircraftClass::APOff)
        {
        ...
        else if (closest == GetPrevTaxiPt(curTaxiPoint) and
                 atcstatus == tTaxiBack)
        {
...
static int EngineStart(AircraftClass *self)
{
    if (self->af->IsSet(AirframeClass::EngineStopped))
    {
        ...
        */
        ...
        if /*(self->af->rpm > 0.2f)*/ (self->af->IsSet(
            AirframeClass::JfsStart)) // Jfs is runnning time to light
        {
    ...
    {
        if /*(self->af->rpm2 > 0.2f)*/ (
            self->af->IsSet(AirframeClass::JfsStart))
        {
...
static struct PreflightActions
{
    ...
    {PreflightActions::EXTLON, AircraftClass::Extl_Main_Power, 1,
     NULL}, //exterior lighting
    {PreflightActions::EXTLON, AircraftClass::Extl_Anti_Coll, 1,
     NULL}, //exterior lighting
    {PreflightActions::EXTLON, AircraftClass::Extl_Wing_Tail, 1,
     NULL}, //exterior lighting
    {PreflightActions::EXTLON, AircraftClass::Extl_Flash, 1,
     NULL}, //exterior lighting
    {PreflightActions::INS, AircraftClass::INS_AlignNorm, 485,
     NULL}, //time to align
    {PreflightActions::INS, AircraftClass::INS_Nav, 0, NULL},
    {PreflightActions::VOLUME, 1, 1, NULL},
    {PreflightActions::VOLUME, 2, 1, NULL},
    {PreflightActions::AFFLAGS, AirframeClass::NoseSteerOn, 3,
     NULL}, //shortly before taxi
...
static const int MAX_PF_ACTIONS =
    sizeof(PreFlightTable) / sizeof(PreFlightTable[0]);
...
int DigitalBrain::PreFlight()
{
    ...
    if (SimLibElapsedTime < mNextPreflightAction)
        return 0;
    ...
    case PreflightActions::FNX:
        ...
        mNextPreflightAction =
            SimLibElapsedTime +
            PreFlightTable[mActionIndex].timedelay * CampaignSeconds;
        break;
    ...
    case PreflightActions::POWERON:
        if (not self->PowerSwitchOn(
                (AircraftClass::AvionicsPowerFlags)PreFlightTable[mActionIndex]
                    .data))
        {
            self->PowerOn(
                (AircraftClass::AvionicsPowerFlags)PreFlightTable[mActionIndex]
    ...
    case PreflightActions::EXTLON:
        if (not self->ExtlState(
                (AircraftClass::ExtlLightFlags)PreFlightTable[mActionIndex]
                    .data))
        {
            self->ExtlOn(
                (AircraftClass::ExtlLightFlags)PreFlightTable[mActionIndex]
    ...
    case PreflightActions::INS:
        ...
        if (PreFlightTable[mActionIndex].data == AircraftClass::INS_AlignNorm)
        {
            ...
            if (self == SimDriver.GetPlayerEntity() and
                OTWDriver.pCockpitManager)
            {
                ...
                OTWDriver.pCockpitManager->mpIcp->SetICPFlag(
                    ICPClass::MODE_LIST);
        ...
        self->INSOn(
            (AircraftClass::INSAlignFlags)PreFlightTable[mActionIndex].data);
        mNextPreflightAction =
            SimLibElapsedTime +
            PreFlightTable[mActionIndex].timedelay * CampaignSeconds;

        if (self->INSState(AircraftClass::INS_Aligned) and
            self->INSState(AircraftClass::INS_AlignNorm))
            self->INSOff(AircraftClass::INS_AlignNorm);

        if (self == SimDriver.GetPlayerEntity() and
            OTWDriver.pCockpitManager and
...
int DigitalBrain::IsAtFirstTaxipoint()
{
    ...
    if (SimLibElapsedTime >
        w->GetWPDepartureTime() - Airbase->brain->MinDeagTime())
    {
        return 0;
    }
    else

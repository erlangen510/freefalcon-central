...
extern float
    g_fFuelBaseProp; // 2002-03-14 S.G. For better fuel consumption tweaking
extern float
    g_fFuelMultProp; // 2002-03-14 S.G. For better fuel consumption tweaking
extern float
    g_fFuelTimeStep; // 2002-03-14 S.G. For better fuel consumption tweaking
extern bool
    g_bFuelUseVtDot; // 2002-03-14 S.G. For better fuel consumption tweaking
extern float
    g_fFuelVtClip; // 2002-03-14 S.G. For better fuel consumption tweaking
extern float
    g_fFuelVtDotMult; // 2002-03-14 S.G. For better fuel consumption tweaking
extern bool
    g_bFuelLimitBecauseVtDot; // 2002-03-14 S.G. For better fuel consumption tweaking
extern float
    g_fWaypointBurnerDelta; // 2002-03-27 MN BurnerDelta in WaypointMode bitand WingyMode
...
float DigitalBrain::TrackPointLanding(float speed)
{
    ...
    elErr = SimpleTrackElevation(trackZ - self->ZPos(),
    ...
    else
    {
        ...
        throtl = eProp * 0.02F + autoThrottle -
                 af->vtDot * SimLibMajorFrameTime * 0.005F;
    }
...
float DigitalBrain::AutoTrack(float maxMnvrGs)
{
    ...
    */
    ...
    {
        // If we're stupid
        if (SkillLevel() < 2 and fabs(ata) > 90.0F and
        ...
        {
            SetRstick(droll * RTD);
            SetPstick(ata * min((30.0F * DTR) / (float)fabs(droll), 1.0F),
                      maxMnvrGs,
                      AirframeClass::AlphaCommand); //me123 from errorcommand
            SetYpedal(0.0F);
...
float DigitalBrain::SetPstick(float pitchError, float gLimit, int commandType)
{
    ...
    if (stickCmd <= 1.0F)
    {
        stickCmd = -(float)sqrt((1.0F - stickCmd) /
                                (4.0F + self->platformAngles.costhe));
    }
    else
    {
        stickCmd = (float)sqrt((stickCmd - 1.0F) /
                               (af->MaxGs() - self->platformAngles.costhe));
    }
...
int DigitalBrain::MachHold(float m1, float m2, int pitchStick)
{
    ...
    if (m1 < (af->MinVcas()) and af->IsSet(AirframeClass::InAir))
    {
        //Cobra 10/31/04 TJL
        m1 =
            af->MinVcas(); // * TJL 08/28/04 FTPSEC_TO_KNOTS; m1 is in KNOTS and MinVcas is in KNOTS
        eProp = m1 - m2;
    }
    ...
    else
    {
        ...
        if (curMode < LoiterMode and
            curMode not_eq
                LandingMode) // 2002-02-12 MODIFIED BY S.G. And not in landing mode
        {
        ...
        else
        {
            ...
            thr = (eProp + g_fFuelBaseProp) *
                  g_fFuelMultProp; /* linear in-between */
            autoThrottle += (eProp - usedVtDot * g_fFuelVtDotMult) *
                            g_fFuelTimeStep * SimLibMajorFrameTime;
            ...
            if ((curMode >= LoiterMode or
                 curMode ==
                     LandingMode) and // 2002-02-12 MODIFIED BY S.G. Don't go in AB if you're in landing mode either
                    m2 >
                        aeroDataset[self->af->VehicleIndex()]
                                .inputData[AeroDataSet::MinVcas] *
                            0.9f and // JB 011018 If we can't keep our speed up, use the buner 2002-02-12 MODIFIED BY S.G. Use a percentage of MinVcas instead.
                    (not flightLead or
                     flightLead and ((AircraftClass*)flightLead)->af) and
                    (not flightLead or
                     (((AircraftClass*)flightLead)->af == af or
                      ((((AircraftClass*)flightLead)->af->rpm <
                        1.0F)) and // JB 011025 If the lead is using his burner, we can use ours 2002-02-12 MODIFIED BY S.G. Don't look at lead's burner or g_fFormationBurnerDistance if we're RTBing...
                          dist < g_fFormationBurnerDistance * NM_TO_FT) or
                     curMode ==
                         RTBMode) or // allow usage of burner if lead is more than defined distance away
                ...
                if (not isWing)
                {
                    ...
                    for (i = 1; i < size; i++)
                    {
                        ...
                        if (flightMember and flightMember->DBrain() and
                            not flightMember->DBrain()->mInPositionFlag)
                            break;
                    }
        ...
        if (targetPtr and HoldCorner(self->CombatClass(), targetPtr))
        {
            ...
            if (not af->IsSet(AirframeClass::IsDigital))
                maxDelta = 0.0F;

            if (pitchStick and cornerDelta > maxDelta and
                pStick > 0.0F) // and fabs(self->Roll()) < 110.0F * DTR)
            {
                pStick *= max(
                    0.55F,
                    (1.0F - (cornerDelta - maxDelta) /
                                (cornerSpeed * 0.75F))); //me123 0.55 from 0.25
            }
        }
    }
    ...
    if (curMode == MissileEngageMode or curMode == GunsEngageMode or
        curMode == WVREngageMode)
    {
        //me123 status test. we are inside 6nm, somebody is pointing at us and we are head on.

        if (targetData and
            not F4IsBadReadPtr(targetData,
                               sizeof(SimObjectLocalData)) and // JB 010318 CTD
    ...
    {
        char tmpchr[128];
        sprintf(tmpchr, "%.0f %.0f %.0f %.3f %.2f %.2f", m1, m2, eProp, thr,
                autoThrottle, throtl);
...
void DigitalBrain::RollOutOfPlane(void)
{
    ...
    if (lastMode not_eq RoopMode)
    {
        ...
        if (self->Roll() >= 0.0)
        {
            newroll =
                self->Roll() -
                30.0F * DTR; //me123 don't do a fucking quarterplane :-) from 45
        }
        else
        {
            newroll =
...
int HoldCorner(int combatClass, SimObjectType* targetPtr)
{
    ...
    if (targetPtr->BaseData()->IsAirplane() or
        targetPtr->BaseData()
            ->IsFlight()) // 2002-02-26 MODIFIED BY S.G. airplane and fligth are ok in here
    {
        // Find the data table for these two types of A/C
        hisCombatClass =
            targetPtr->BaseData()
                ->CombatClass(); // 2002-02-26 MODIFIED BY S.G. Removed the AircraftClass cast
        theIntercept =
            &(DigitalBrain::maneuverData[combatClass][hisCombatClass]);

...
const DWORD NonTargets[] = {TYPE_TREE, TYPE_FENCE, TYPE_VASI, TYPE_TAXI_SIGN,
                            0};
...
void DigitalBrain::AiRunDecisionRoutines(void)
{
    ...
    ShiAssert(flightLead)
        ...
        if (flightLead) AiCheckForUnauthLand(flightLead->Id());
...
void DigitalBrain::AiRunTargetSelection(void)
{
    ...
    if (curSpike or
        (flightLead and flightLead not_eq self and flightLead->IsSPJamming()))
    {
...
#endif
                AiSearchTargetList(
                    pnewTarget); // Run targeting for wingman with designated target
            }
            else
            {
                mDesignatedObject = FalconNullId;
                 ...
                 mDesignatedObject ==
                     FalconNullId) // If we are ordered to scan for targets
        {
            TargetSelection(); // Run the full selection routine
        }
        else
        {
            ShiAssert(curMode not_eq
                      GunsEngageMode); // Otherwise just chill out
...
void DigitalBrain::AiSearchTargetList(VuEntity *pentity)
{
    ...
    if (((FalconEntity *)pentity)->IsSim())
    {
        ...
        if (not theTarget->OnGround() and
            (theTarget->IsDead() or theTarget->IsEject()))
            theTarget = NULL;
            ...
            if (theTargetGroup->OnGround())
            {
                ...
                if (groundTargetPtr and groundTargetPtr->BaseData()->IsSim() and
                    ((SimBaseClass *)groundTargetPtr->BaseData())
                            ->GetCampaignObject() == theTargetGroup and
                    not groundTargetPtr->BaseData()->IsExploding() and
                    not groundTargetPtr->BaseData()->IsDead() and
                    ((SimBaseClass *)groundTargetPtr->BaseData())->pctStrength >
                        0.0f)
                    return;

                // Use our hep function
                theTarget = FindSimGroundTarget(
                    theTargetGroup, theTargetGroup->NumberOfComponents(), 0);
            }
            // Target is not on the ground, select your opponent in the flight, wrapping if the opponent has less planes than us
            else
                theTarget = theTargetGroup->GetComponentEntity(
                    isWing % theTargetGroup->NumberOfComponents());
    ...
    if (theTarget)
    {
        ...
        if ((not targetPtr or targetPtr->BaseData() not_eq theTarget) and
            (not groundTargetPtr or
             groundTargetPtr->BaseData() not_eq theTarget))
        {
    ...
    else
    {
        ...
        ClearTarget();
        mDesignatedObject =
            FalconNullId; // 2002-04-04 ADDED BY S.G. If we are clearing the target, we might as well clear the designated target as well so we leave it alone...
        AddMode(WaypointMode);
    }
}
                                                ...
                                                int startPos)
{
    ...
    for (i = 0; i < usComponents; i++)
    {
        ...
        flightMember[i] =
            (AircraftClass *)self->GetCampaignObject()->GetComponentEntity(i);

        if (flightMember[i] and
            (flightMember[i]->IsDigital() or flightMember[i]->IsLocal()))
        {
            ...
            else
            {
                ...
                for (int j = 0;
                     not haveHARMS and not(otherHaveHARMS and i > isWing) and
                     flightMember[i]->Sms and
                     j < flightMember[i]->Sms->NumHardpoints();
                     j++)
                {
                    if (flightMember[i]->Sms->hardPoint[j]->weaponPointer and
                        flightMember[i]->Sms->hardPoint[j]->GetWeaponClass() ==
                            wcHARMWpn)
                    {
                        if (i == isWing)
                            haveHARMS = TRUE;
    ...
    if (not haveHARMS and not hasAGMissile and targetNumComponents)
    {
        ...
        if (startPos == 0)
        {
            ...
            for (i = startPos; i < targetNumComponents and startPos > 0; i++)
                if (not targetGroup->GetComponentEntity(i)->IsVehicle() or
                    not targetGroup->GetComponentEntity(i)->OnGround())
                    startPos = 0;
        }
         ...
         i = startPos ? 0 : i + 1, startPos = 0)
    {
        ...
        if (not simTarg or
            F4IsBadReadPtr(simTarg, sizeof(simTarg))) //sanity check
            continue;
        ...
        if (simTarg->IsStatic()) // It's a feature
        {
            ...
            if (fc and not F4IsBadReadPtr(fc, sizeof(fc)) and
                fc->Priority > 2) // higher priority number = lower priority
                continue;
        ...
        for (j = 0; j < usComponents; j++)
        {
            if (flightMember[j])
            {
                ...
                if (flightMember[j]->DBrain() and
                    ((flightMember[j]->DBrain()->groundTargetPtr and
                      flightMember[j]->DBrain()->groundTargetPtr->BaseData() ==
                          simTarg) or
                     flightMember[j]->DBrain()->gndTargetHistory[0] ==
                         simTarg or
        ...
        if ((simTarg->IsVehicle() and
             ((SimVehicleClass *)simTarg)->GetRadarType() not_eq
                 RDR_NO_RADAR) or // It's a vehicle and it has a radar
            (simTarg->IsStatic() and
             ((SimStaticClass *)simTarg)->GetRadarType() not_eq
                 RDR_NO_RADAR)) // It's a feature and it has a radar
        {
            ...
            if ((((FlightClass *)self->GetCampaignObject())->shotAt ==
                     targetGroup and
                 ((FlightClass *)self->GetCampaignObject())->whoShot == self) or
                ((((FlightClass *)self->GetCampaignObject())->whoShot ==
                  NULL) and
                 (haveHARMS or not otherHaveHARMS)))
            {
                gotRadar = TRUE;
        ...
        else
        {
            ...
            if (simTarg->IsGroundVehicle() and
                ((GroundClass *)simTarg)->isAirDefense)
            {
                gotRadar = TRUE;
                firstSimTarg = simTarg; // Yes, use it for a target
    ...
    if (startPos < targetNumComponents)
    {
        ...
        if (not gotRadar and
            (unsigned)((unsigned)rand() % (unsigned)32) >
                (unsigned)((unsigned)SkillLevel() + (unsigned)28))
            firstSimTarg =
...
SimBaseClass *DigitalBrain::FindSimAirTarget(CampBaseClass *targetGroup,
                                             int targetNumComponents,
                                             int startPos)
{
    int i, gotRadar = FALSE;
    ...
    for (i = 0; i < usComponents; i++)
    {
        ...
        if (flightMember[i] and (not flightMember[i]->IsDigital() and
                                 not flightMember[i]->IsLocal()))
        {
            flightMember[i] = NULL;
        }
    }
         ...
         i = startPos ? 0 : i + 1, startPos = 0)
    {
        ...
        for (j = 0; j < usComponents;
             j++) // MN - use the gndtargethistory for divert air targets, too.
        {
            // Cobra - You will always get a match with my last target...skip me
            if (flightMember[j]->vehicleInUnit == self->vehicleInUnit)
                continue;

            if (flightMember[j] and flightMember[j]->DBrain() and
                ((flightMember[j]->DBrain()->airtargetPtr and
                  flightMember[j]->DBrain()->airtargetPtr->BaseData() ==
                      simTarg) or
...
SimBaseClass *DigitalBrain::FindJSOWGroundTarget(CampBaseClass *targetGroup,
                                                 int targetNumComponents,
                                                 int feaNo)
{
    int usComponents = self->GetCampaignObject()->NumberOfComponents();
    ...
    if (i < targetNumComponents)
    {
        // Now after all this, see if the AI is too dumb for selecting a valid target...
        // If he is, select at random
        if ((unsigned)((unsigned)rand() % (unsigned)32) >
            (unsigned)((unsigned)SkillLevel() + (unsigned)28))
        {
...
int DigitalBrain::FindJDAMGroundTarget(CampBaseClass *targetGroup,
                                       int targetNumComponents, int feaNo)
{
    int usComponents = self->GetCampaignObject()->NumberOfComponents();
    int FeatureNumber = feaNo;
    FeatureClassDataType *fc = NULL;
...
void DigitalBrain::AiCheckEngage(void)
{
    ...
    if ((mpActionFlags[AI_ENGAGE_TARGET] or
         mWeaponsAction == AI_WEAPONS_FREE) and
        mpActionFlags[AI_EXECUTE_MANEUVER] not_eq TRUE + 1)
    {
        MergeCheck();
        BvrEngageCheck();
        GunsEngageCheck();
...
void DigitalBrain::AiCheckLandTakeoff(void)
{
    ...
    else if (mpActionFlags[AI_LANDING] or
             (atcstatus >= lReqClearance and atcstatus <= lCrashed))
    {
    ...
    else if (self->curWaypoint->GetWPAction() == WP_LAND and
             not self->OnGround() and distAirbase < 30.0F * NM_TO_FT and
             (missionComplete or IsSetATC(SaidRTB) or IsSetATC(SaidBingo) or
              mpActionFlags
                  [AI_RTB])) // don't land if one of these conditions isn't met
    {
        if (atcstatus > lIngressing)
            mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;
    ...
    else if (self->curWaypoint->GetWPAction() == WP_TAKEOFF and
             self->OnGround())
    {
        mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;
        AddMode(TakeoffMode);
...
void DigitalBrain::AiCheckManeuvers(void)
{
    ...
    if (mpActionFlags
            [AI_EXECUTE_MANEUVER]) // If we are ordered to do a maneuver
    {
...
void DigitalBrain::AiCheckFormation(void)
{
    ...
    if (mpActionFlags[AI_ENGAGE_TARGET] and
        not mpActionFlags[AI_EXECUTE_MANEUVER] and
        (agDoctrine not_eq AGD_NONE or groundTargetPtr))
    {
        AddMode(WaypointMode);
    }
...
{
    if (vuDatabase->Find(from) ==
        self->GetCampaignObject()
            ->GetComponentLead()) // if from the flight lead
    {
...
void DigitalBrain::AiCheckForUnauthLand(VU_ID lead)
{
    ...
    if (not self->OnGround() and leader and
        mpActionFlags[AI_FOLLOW_FORMATION] == TRUE and
        leader->DBrain()->ATCStatus() < tReqTaxi and leader->OnGround() and
        atcstatus == noATC)
    {

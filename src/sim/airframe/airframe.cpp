...
void AirframeClass::Init(int idx)
{
    ...
    else
    {
        ...
        platform->ClearAcStatusBits(
            AircraftClass::ACSTATUS_EXT_LIGHTS bitor
            AircraftClass::ACSTATUS_EXT_NAVLIGHTS |
            AircraftClass::ACSTATUS_EXT_NAVLIGHTSFLASH bitor
...
void AirframeClass::Exec(void)
{
    ...
    if (simpleMode == SIMPLE_MODE_AF and not IsSet(AirframeClass::OnObject))
    {
        // JB carrier
        Atmosphere();
        Aerodynamics();
...
void AirframeClass::RemoteUpdate()
{
    ...
    groundZ = OTWDriver.GetGroundLevel(x, y, &gndNormal);
    float mag =
        (float)sqrt(gndNormal.x * gndNormal.x + gndNormal.y * gndNormal.y +
    ...
    platform->dmx[1][0] =
        pa->cospsi * pa->sinthe * pa->sinphi - pa->sinpsi * pa->cosphi;
    platform->dmx[1][1] =
        pa->cospsi * pa->cosphi + pa->sinpsi * pa->sinthe * pa->sinphi;
    platform->dmx[1][2] = pa->costhe * pa->sinphi;

    platform->dmx[2][0] =
        pa->sinpsi * pa->sinphi + pa->cospsi * pa->sinthe * pa->cosphi;
    platform->dmx[2][1] =
        -pa->cospsi * pa->sinphi + pa->sinpsi * pa->sinthe * pa->cosphi;
    platform->dmx[2][2] = pa->costhe * pa->cosphi;

    vcas = get_air_speed(vt * FTPSEC_TO_KNOTS,
                         -1 * FloatToInt32(platform->ZPos()));

    pstick =
        0.0F; // (q*vt + platform->platformAngles.cosmu * platform->platformAngles.cosgam * 0.5F)/9.0F;
...
void AirframeClass::SetStallConditions(void)
{
    ...
    if (platform->IsF16() and platform->AutopilotType() == AircraftClass::APOff)
    {
        switch (stallMode)
        {
        case None:
            ...
            {
                zpdamp =
                    (avgPdelta * (0.05F + avgRdelta + avgYdelta)) / vcas * 5.0F;
            ...
            if (not IsSet(Simplified) and
                ((not platform->OnGround() and vcas < 180.0f) or
                 ...
                 (fabs(pshape) > 0.6F and fabs(rshape) > 0.6F and
                  IsSet(LowSpdHorn))))
            {
                ...
                if ((not platform->OnGround() and vcas < 60.0f or
                     not platform->OnGround() and fabs(alpha) > 18.0F and
                         vcas < 60.0f + 60 * (loadingFraction - 1.3F) +
                                    10 * fabs(assymetry / weight) * 10.0F) or

                    alpha > 31.0F -
                                9 * (loadingFraction - 1.3F) - //me123 addet 9
                                //me123 bulshit IsSet(LowSpdHorn)*3.0F
                                //me123 - IsSet(CATLimiterIII)*5.0F
                                -fabs(assymetry / weight) *
                                    10.0F or // 10 from 5 me123
                    alpha < -14.0F + (loadingFraction - 1.3F) +
                                IsSet(LowSpdHorn) * 3.0F +
                                fabs(assymetry / weight) * 5.0F)
                {

                    stallMode = EnteringDeepStall;

                    if (alpha < 0.0F)
                        pitch = (float)fabs(platform->platformAngles.singam) *
                                platform->platformAngles.cosphi * qbar /
                                -10.0F * DTR;
                    else
                        pitch = (float)fabs(platform->platformAngles.singam) *
                                platform->platformAngles.cosphi * qbar /
                    ...
                    stallMagnitude +=
                        (1.0f - rand() / (float)RAND_MAX * 2.0f) * 5.0f;
                    stallMagnitude *= loadingFraction;

                    if (platform->platformAngles.cosgam *
                            platform->platformAngles.sinmu <
                        0.0F)
        ...
        case EnteringDeepStall:
            if (platform->platformAngles.sinthe < -0.766044F and
                fabs(alpha * (rand() / (float)RAND_MAX +
                              platform->platformAngles.cosphi)) < 15.0F)
            {
                stallMode = Recovering;
            }
            else if (platform->platformAngles.sinthe < 0.342F and
                     fabs(platform->platformAngles.cosphi) > 0.82F)
            {
                if (alpha > 0.0f and
                    fabs(alpha - (60.0f + oscillationTimer * stallMagnitude *
                                              max(0.0F, (0.3F - fabs(r)) *
                                                            3.3F))) < 10.0F and
                ...
                else if (alpha < 0.0f and
                         fabs(alpha -
                              (-40.0f + oscillationTimer * stallMagnitude *
                                            max(0.0F, (0.3F - fabs(r)) *
                                                          3.3F))) < 10.0F and
                         oscillationSlope * q > 0.0F)
                {
        ...
        case DeepStall:
            ...
            if (stallMode == DeepStall and fabs(slice) > 0.3F and
                alpha < -10.0F)
            {
                stallMode = Spinning;
                oldp02[5] = alpha;
        ...
        case Recovering:
            ...
            if ((alpha > g_fRecoveryAOA or alpha < -11.0f) and
                platform->platformAngles.sinthe > 0.342F)
            {
                stallMode = EnteringDeepStall;
                stallMagnitude +=
                    (1.0f - rand() / (float)RAND_MAX * 2.0f) * 5.0f;
        ...
        if (platform->platformAngles.sinthe > .707 and
                1.8888F * theta * RTD + 45.0F > vcas or
            (g_bRealisticAvionics and gearPos > 0.9F and
             cockpitFlightData.alpha >= 15.0F))
        {
            ...
            if (not IsSet(HornSilenced))
            {
                ...
                platform->SoundPos.Sfx(
                    auxaeroData->sndLowSpeed); // MLR 5/16/2004 -
            }
        }
...
void AirframeClass::AddWeapon(float Weight, float DragIndex, float offset)
{
    ...
    mass += Weight / GRAVITY;
    dragIndex +=
        DragIndex; //*(18238.0F/emptyWeight); //equate for relative size differences
...
float AirframeClass::CalcThrotlPos(float speed)
{
    ...
    th1 = Math.TwodInterp(mz, tempMach, engineData->alt, engineData->mach,
                          engineData->thrust[0], engineData->numAlt,
                          engineData->numMach, &EngAltBreak, &EngMachBreak);
    th2 = Math.TwodInterp(mz, tempMach, engineData->alt, engineData->mach,
    ...
    newthrotl = min(
        ((drag + Mu_fric * (1.0F - nzcgs) * GRAVITY) / ethrst * mass - th1) /
            (th2 - th1) * accel,
        1.5F);
...
float AirframeClass::CalcDesSpeed(float desAlpha)
{
    ...
    if ((cl + (desAlpha - oldp03[2]) * clalpha) < 0.0F)
        ...
        return (float)sqrt(
            2.0F * GRAVITY * mass /
            (area * rho * (cl + (desAlpha - oldp03[2]) * clalpha)));
}
...
int AirframeClass::AddFuel(float lbs)
{
    ...
    for (int i = center + 1; i < platform->Sms->NumHardpoints(); i++)
    {
        if (platform->Sms->hardPoint[i] and
            platform->Sms->hardPoint[i]->GetWeaponType() not_eq wtNone)
            weapons++;
    }
...
{
    float lbs = aeroDataset[vehicleIndex].inputData[AeroDataSet::InternalFuel] -
                fuel; // JPO - use the constant...
    weight += lbs;
    mass += lbs / GRAVITY;
...
void AirframeClass::ToggleHook(void)
{
    ...
    if (IsSet(Hook))
    {
        ...
        if (faultSys and faultSys->GetFault(hook_fault) and
            ((AircraftClass*)(SimDriver.GetPlayerEntity()))
                ->af->platform->IsF16())
    ...
    else
    {
        ...
        // MD -- 20031006: setting the fault was done in the cockpit callback but it will work better
        // here and ensure the shared memory state is updated even when we aren't looking at the panel
        if (faultSys and ((AircraftClass*)(SimDriver.GetPlayerEntity()))
                             ->af->platform->IsF16())
...
{
    int pos =
        auxaeroData->tefNStages / 2 + (auxaeroData->tefNStages % 2) ? 1 : 0;
...
void AirframeClass::SetFlaps(bool islanding)
{
    ...
    if (islanding) // we want to get some flaps down
    {
        ...
        else // interpolate
        {
            ...
            int stage = static_cast<int>(auxaeroData->tefNStages -
                                         vdiff * auxaeroData->tefNStages - 1);
            ...
            {
                stage = static_cast<int>(auxaeroData->lefNStages -
                                         (vdiff * auxaeroData->lefNStages) - 1);
                stage = max(0, min(stage, auxaeroData->lefNStages));
                lefPos =
                    stage * auxaeroData->lefMaxAngle / auxaeroData->lefNStages;
            }
        }
    }
...
void AirframeClass::SetFlapsPlayer()
{
    ...
    //F15A/B, C, E
    if (auxaeroData->typeAC == 3 or auxaeroData->typeAC == 4 or
        auxaeroData->typeAC == 5)
    {
    ...
    if (auxaeroData->typeAC == 8 or auxaeroData->typeAC == 9 or
        auxaeroData->typeAC == 10)
    {
        if (mach > 1.05)
        {
            tefPos = 0;
    ...
    if (g_bEnableRandomFailures and platform->IsPlayer())
    {
        ...
        if (MTBF)
        {
            if (NextFailure == 0)
            {
                ...
                NextFailure = static_cast<VU_TIME>(
                    vuxGameTime +
                    ((rand() / (float)RAND_MAX) * (MTBF * 7200000.0f)));

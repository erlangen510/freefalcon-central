...
protected:
    ...
    SimObjectType*
        agThreatTargetPtr; //Cobra Let's target those pesky SAMs, yet keep a normal GTPtr
    ...
    BOOL madeAGPass;
    float
        AGattackAlt; // Cobra - holder for current weapon ground attack altitude
...
protected:
    ...
    //ATC stuff
    VU_TIME
    createTime; //when this brain was created in terms of SimLibElapsedTime
    ...
    AtcStatusEnum
        atcstatus; //at what point in the takeoff/landing process are you?
    int curTaxiPoint; //index into PtDataTable, 0 means none
    float
        desiredSpeed; //speed at which we want to move so we arrive at next point on time
    ...
    SimObjectType* InsertIntoTargetList(SimObjectType* root,
...
public:
    ...
    float PIDLoop(float error, float K, float KD, float KI, float Ts,
    ...
    int WingmanTakeRunway(ObjectiveClass* Airbase, AircraftClass* flightLead,
...
public:
    ...
    SimBaseClass* FindSimAirTarget(CampBaseClass* targetGroup,
                                   int targetNumComponents, int startPos);
...
public:
    ...
    VU_ID
    escortFlightID; // 2002-02-27 ADDED BY S.G. Instead of calculating it everytime in BVR, let's find it here...
    ...
    unsigned int
        moreFlags; // 2002-03-08 ADDED BY S.G. atcFlags ran out of bits...

    // Cobra help function to search for a JDAM ground target
    int FindJDAMGroundTarget(CampBaseClass* targetGroup,
                             int targetNumComponents, int feaNo);
    SimBaseClass* FindJSOWGroundTarget(CampBaseClass* targetGroup,
                                       int targetNumComponents, int feaNo);
    /////////////////////////////////////////////////////////
};

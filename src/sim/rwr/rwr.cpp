...
void RwrClass::DrawSymbol(VirtualDisplay* display, int symbolID, int boxed)
{
    ...
    if (flash and
        ((oldcount and show not_eq 0) or (not oldcount and show == 0)))
    {
        ++show;
    ...
    if (not flash and
        ((oldcount and show == 0) or (not oldcount and show not_eq 0)))
    {
        ++show;
    ...
    static const float basicAir[][2] = {{0.00F, 0.06F},
                                        {0.09F, -0.03F},
                                        {0.00F, 0.01F},
                                        {-0.09F, -0.03F},
                                        {0.00F, 0.06F}};
    static const int NUM_BASIC_AIR_POINTS =
        sizeof(basicAir) / sizeof(basicAir[0]);

    static const float advAir[][2] = {
        {0.00F, 0.09F},  {0.09F, -0.01F},  {0.03F, 0.01F}, {0.00F, -0.06F},
        {-0.03F, 0.01F}, {-0.09F, -0.01F}, {0.00F, 0.09F}};
    static const int NUM_ADV_AIR_POINTS = sizeof(advAir) / sizeof(advAir[0]);

    static const float theBoat[][2] = {
        {-0.06F, -0.04F}, {-0.08F, 0.00F}, {-0.03F, 0.00F},
        {-0.03F, 0.04F},  {0.03F, 0.04F},  {0.03F, 0.00F},
        {0.06F, 0.00F},   {0.06F, -0.04F}, {-0.06F, -0.04F}};
    ...
    case RWRSYM_ADVANCED_INTERCEPTOR: //
        for (i = 1; i < NUM_ADV_AIR_POINTS; i++)
            display->Line(advAir[i][0], advAir[i][1], advAir[i - 1][0],
                          advAir[i - 1][1]);
    ...
    case RWRSYM_BASIC_INTERCEPTOR:
        for (i = 1; i < NUM_BASIC_AIR_POINTS; i++)
            display->Line(basicAir[i][0], basicAir[i][1], basicAir[i - 1][0],
                          basicAir[i - 1][1]);
    ...
    case RWRSYM_NAVAL:
        for (i = 1; i < NUM_BOAT_POINTS; i++)
            display->Line(theBoat[i][0], theBoat[i][1], theBoat[i - 1][0],
                          theBoat[i - 1][1]);
    ...
    case RWRSYM_MIB_F_BW:
        if (not show)
            ...
            for (i = 1; i < NUM_BASIC_AIR_POINTS; i++)
                display->Line(basicAir[i][0], basicAir[i][1],
                              basicAir[i - 1][0], basicAir[i - 1][1]);

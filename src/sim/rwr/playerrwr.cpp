    ...
    if (g_bRealisticAvionics and SimDriver.GetPlayerAircraft())
    {
        if (IO.AnalogIsUsed(AXIS_THREAT_VOLUME) == false) // Retro 3Jan2004
        {
            ...
            if (SimDriver.GetPlayerAircraft()->ThreatVolume == 8)
            {
                ...
                Vol =
                    -(float)(SimDriver.GetPlayerAircraft()->ThreatVolume * 250);
        }
        else // Retro 3Jan2004
        {
            ...
            Vol = -(float)(/*15000-*/ IO.GetAxisValue(AXIS_THREAT_VOLUME)) /
                  1.5F; // Retro 26Jan2004
        }
    }
...
SimObjectType* PlayerRwrClass::Exec(SimObjectType* targetList)
{
    ...
    while (curObj and
           not F4IsBadReadPtr(curObj, sizeof(SimObjectType))) // JB 010318 CTD
    {
        ...
        if ((curObj->BaseData()) and
            not F4IsBadReadPtr(curObj->BaseData(), sizeof(FalconEntity)) and
            (curObj->BaseData()->IsSim()) and
            (curObj->BaseData()->GetRadarType())) // JB 010318 CTD
        {
            // Localize the info
            ...
            if (SimLibElapsedTime >
                localData->sensorLoopCount[RWR] +
                    0.95F * curSimObj->RdrCycleTime() * SEC_TO_MSEC)
            {
                // Can we hear it?
                if (not IsFiltered(curObj->BaseData()))
                {
                    if (/*curObj->localData->range <= 5 * NM_TO_FT and curObj->localData->ataFrom < 60.0f * DTR and //me123 addet range check*/
                        CanSeeObject(curObj) and BeingPainted(curObj) and
                        CanDetectObject(curObj))
                    {
                        if (ObjectDetected(curObj->BaseData(), Track_Ping) and
                            newGuy >= 0)
                        {
                            newGuy = TRUE;
                        }
    ...
    if ((Camp_GetCurrentTime() - platformCampObj->GetSpotTime() <=
             ReconLossTime[platformCampObj->GetMovementType()] / 8 and
         ((platformCampObj->GetSpotted() >> enemy) bitand 0x01)))
        spotted = 1;
    else
    {
    ...
    while (curEmitter)
    {
        ...
        if (not IsFiltered(curEmitter) and
            ...
            ((curEmitter->IsObjective() and
              curEmitter->GetElectronicDetectionRange(Air) not_eq 0) or
             // An objective (sim objectives have no sensor routine
             // so they can never make it to the contact list by themself)
             // that has a working radar or a campaign thing
             curEmitter->IsAggregate()) and
        ...
        if (not spotted)
        {
            if (roea[curEmitter->GetTeam()] == ROE_ALLOWED)
            {
                ...
                if (curEmitter->IsObjective() and
                    curEmitter->CanDetect(platformCampObj) and
                    ((ObjectiveClass*)curEmitter)->IsGCI())
                {
                    if (not platformCampObj->GetSpotted(enemy))
                        RequestIntercept((Flight)platform->GetCampaignObject(),
                                         enemy);

                    platformCampObj->SetSpotted(
                        enemy, TheCampaign.CurrentTime,
                        ((ObjectiveClass*)curEmitter)->HasNCTR() not_eq
                            0); // 2002-02-11 MODIFIED BY S.G. Ground objective radar can identify me
                    spotted = 1;
                }
            }
        }
    ...
    if (OTWDriver.DisplayInCockpit())
    {
        for (i = 0; i < numContacts; i++)
        {
            if (detectionList[i].missileActivity)
            {
                ...
                if (detectionList[i].entity and
                    (not((MissileClass*)detectionList[i].entity)->IsMissile() or
                     ((MissileClass*)detectionList[i].entity)
                             ->GetSeekerType() not_eq SensorClass::Radar))
                {
    ...
    for (i = 0, count = 0; i < numContacts and count < 40; i++)
    {
        ...
        {
            cockpitFlightData.RWRsymbol[count] =
                detectionList[i]
                    .radarData->RWRsymbol; // Which symbol shows up on the RWR
            cockpitFlightData.bearing[count] = detectionList[i].bearing;
            cockpitFlightData.missileActivity[count] =
                detectionList[i]
                    .missileActivity; // Is launching on us (active OR beam rider)
            cockpitFlightData.missileLaunch[count] =
                detectionList[i]
                    .missileLaunch; // Is launching on us (active OR beam rider)
            cockpitFlightData.selected[count] =
                detectionList[i]
                    .selected; // Is launching on us (active OR beam rider)
            cockpitFlightData.lethality[count] =
                detectionList[i]
                    .lethality; // Is launching on us (active OR beam rider)
            count++;
        }
    }
    ...
    if (g_bRealisticAvionics)
    {
        ...
        else if (SimDriver.GetPlayerAircraft() and
                 SimDriver.GetPlayerAircraft()->HasPower(
                     AircraftClass::EWSRWRPower))
        {
            if (SimDriver.GetPlayerAircraft()->EWSPGM() not_eq
                    AircraftClass::Off and
                SimDriver.GetPlayerAircraft()->EWSPGM() not_eq
                    AircraftClass::Stby and
                SimDriver.GetPlayerAircraft()->EWSPGM() not_eq
                    AircraftClass::Man)
                CheckEWS();
        }
...
void PlayerRwrClass::AutoSelectAltitudePriority(void)
{
    ...
    float agl = OTWDriver.GetGroundLevel(platform->XPos(), platform->YPos()) -
                platform->ZPos();
...
void PlayerRwrClass::Display(VirtualDisplay* activeDisplay)
{
    ...
    for (i = 0, drawn = 0; i < numContacts and drawn < last; i++)
    {
        ...
        if (((SimBaseClass*)detectionList[i].entity)->IsSim() and
            ((BattalionClass*)((SimBaseClass*)detectionList[i].entity)
                 ->GetCampaignObject()))
            mode = ((BattalionClass*)((SimBaseClass*)detectionList[i].entity)
                        ->GetCampaignObject())
                       ->GetRadarMode();
        else
            mode = ((SimBaseClass*)detectionList[i].entity)->GetRadarMode();
...
void PlayerRwrClass::DrawContact(DetectListElement* record)
{
    ...
    RadarDataSet* radarfileData =
        &radarDatFileTable[record->radarData->RDRDataInd];
    ...
    if (record->entity and not record->cantPlay)
    {
        ...
        if (record->lethality > 1.0)
            radius =
                (2.0F - record->lethality) *
                0.95f; // 0.95 keeps things from leaking off the edge of the display
        else
            radius =
                (1.0F - record->lethality) *
                0.95f; // 0.95 keeps things from leaking off the edge of the display
        ...
        if (g_bIFFRWR and
            (record->radarData->RWRsymbol == RWRSYM_ADVANCED_INTERCEPTOR or
             record->radarData->RWRsymbol == RWRSYM_BASIC_INTERCEPTOR) and
            not GetRoE(record->entity->GetTeam(), platform->GetTeam(),
                       ROE_AIR_ENGAGE))
        {
        ...
        if (((SimBaseClass*)record->entity)->IsSim() and
            ((BattalionClass*)((SimBaseClass*)record->entity)
                 ->GetCampaignObject()))
            mode = ((BattalionClass*)((SimBaseClass*)record->entity)
                        ->GetCampaignObject())
                       ->GetRadarMode();
        else
        ...
        if (mode == FEC_RADAR_SEARCH_1 and radarfileData->Rwrsymbolsearch1)
        {
            if ((radarfileData->Rwrsymbolsearch1 not_eq RWRSYM_SEARCH) or
                radarfileData->Rwrsymbolsearch1 == RWRSYM_SEARCH and
                    ShowSearch())
        ...
        else if (mode == FEC_RADAR_SEARCH_2 and radarfileData->Rwrsymbolsearch2)
        {
            if ((radarfileData->Rwrsymbolsearch2 not_eq RWRSYM_SEARCH) or
                radarfileData->Rwrsymbolsearch2 == RWRSYM_SEARCH and
                    ShowSearch())
        ...
        else if (mode == FEC_RADAR_SEARCH_3 and radarfileData->Rwrsymbolsearch3)
        {
            if ((radarfileData->Rwrsymbolsearch3 not_eq RWRSYM_SEARCH) or
                radarfileData->Rwrsymbolsearch3 == RWRSYM_SEARCH and
                    ShowSearch())
            ...
            symbol = record->radarData->RWRsymbol;
        ...
        if ((((SimBaseClass*)record->entity)->IsAirplane()) and
...
// JB 010718
void PlayerRwrClass::DoAudio(
    void) // 2002-02-20 S.G. Just flagging it as not currently being used so there are no confusion
{
    if (not g_bRWR) // JB 010802
        return;
    ...
    {
        int SEARCH_PERIOD =
            (int)(((SimBaseClass*)detectionList[i].entity)->RdrCycleTime() *
                  SEC_TO_MSEC);
        ...
        if (detectionList[i].playIt or
            (not detectionList[i].cantPlay and
             (not detectionList[i].entity->IsSim() or
              not detectionList[i].entity->IsDead())))
        {
            if ((detectionList[i].missileActivity) or
                (detectionList[i].playIt) or (detectionList[i].newDetection) or
                (detectionList[i].selected and
                 ((detectionList[i].isLocked) or
                  ((SimLibElapsedTime - detectionList[i].lastPlayed) >
                   (unsigned int)SEARCH_PERIOD))))
            {
                ...
                PlayRWRSoundFX(detectionList[i].radarData->RWRsound, FALSE,
                               0.0f, 1.0f);
...
void PlayerRwrClass::DoAudio(DetectListElement* record)
{
    ...
    if (((SimBaseClass*)record->entity)
            ->IsSim()) //this is a vhehicle not a battalion
    {
        SEARCH_PERIOD = ((SimBaseClass*)record->entity)->RdrCycleTime() *
                        SEC_TO_MSEC; //me123
    }
    ...
    if (((SimBaseClass*)record->entity)->IsSim() and
        ((BattalionClass*)((SimBaseClass*)record->entity)->GetCampaignObject()))
        mode = ((BattalionClass*)((SimBaseClass*)record->entity)
                    ->GetCampaignObject())
                   ->GetRadarMode();
    else
    ...
    case FEC_RADAR_SEARCH_100:
        if (record->isLocked)
            ...
            SEARCH_PERIOD =
                ((SimBaseClass*)record->entity)->RdrCycleTime() * SEC_TO_MSEC;

        break;

    case FEC_RADAR_AQUIRE:
        SEARCH_PERIOD =
            max(((SimBaseClass*)record->entity)->RdrCycleTime() * SEC_TO_MSEC,
                radarfileData->Sweeptimeacuire); //me123 from 1
        break;
    ...
    if (record->playIt or
        (not record->cantPlay and
         (not record->entity->IsSim() or not record->entity->IsDead())))
    {
        ...
        if ( //(record->missileActivity) or
            ...
            (record->selected and
             //((record->isLocked)) or
             ((float)SimLibElapsedTime - (float)record->lastPlayed >
              SEARCH_PERIOD)))
        {
            ...
            {
                if (radarfileData->Rwrsoundacuire)
                    sound = radarfileData->Rwrsoundacuire;
                else
                    sound = record->radarData->RWRsound;

                //F4SoundFXSetDist( record->radarData->RWRsound, FALSE, 0.0f, 1.0f );// PLAY THE TRACKER
                PlayRWRSoundFX(record->radarData->RWRsound, FALSE, 0.0f,
                               1.0f); // PLAY THE TRACKER
            }
            else if (mode == FEC_RADAR_SEARCH_1 or mode == FEC_RADAR_SEARCH_2 or
                     mode == FEC_RADAR_SEARCH_3)
            {
                ...
                {
                    if (radarfileData->Rwrsymbolsearch1 == RWRSYM_SEARCH and
                        not ShowSearch())
                        return;
                ...
                {
                    if (radarfileData->Rwrsymbolsearch2 == RWRSYM_SEARCH and
                        not ShowSearch())
                        return;
                ...
                if (radarfileData->Rwrsoundguide)
                    sound = radarfileData->Rwrsoundguide;
...
void PlayerRwrClass::DrawGrid(void)
{
    ...
    for (i = 0; i < 20; i++)
    {
        display->Line(tick[i * 2][0], tick[i * 2][1], tick[i * 2 + 1][0],
                      tick[i * 2 + 1][1]);
    }
}
...
void PlayerRwrClass::CheckEWS(void)
{
    ...
    if (F4IsBadReadPtr(SimDriver.GetPlayerAircraft(),
                       sizeof(AircraftClass))) // JB 010408 CTD
        return;
    ...
    if (LaunchIndication() and not LaunchDetected)
    {
        ...
        if (OTWDriver.pCockpitManager->mpIcp
                    ->iCHAFF_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] >
                1 or
            OTWDriver.pCockpitManager->mpIcp
                    ->iFLARE_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] > 1)
        {
            LaunchDetected = TRUE;
        }
    }
    ...
    {
        if ((unsigned int)SimDriver.GetPlayerAircraft()->ChaffCount >=
                OTWDriver.pCockpitManager->mpIcp
                    ->iCHAFF_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] and
            SimDriver.GetPlayerAircraft()->ChaffSalvoCount > 0 and
            not ChaffCheck)
        {
            //Set our timer
            SimDriver.GetPlayerAircraft()->ChaffBurstInterval =
                (VU_TIME)(SimLibElapsedTime +
                          (OTWDriver.pCockpitManager->mpIcp->fCHAFF_SI
                               [SimDriver.GetPlayerAircraft()->EWSProgNum] *
                           CampaignSeconds));
            //Reset our count
        ...
        if ((unsigned int)SimDriver.GetPlayerAircraft()->FlareCount >=
                OTWDriver.pCockpitManager->mpIcp
                    ->iFLARE_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] and
            SimDriver.GetPlayerAircraft()->FlareSalvoCount > 0 and
            not FlareCheck)
        {
            //Reset our count
            SimDriver.GetPlayerAircraft()->FlareCount = 0;
            //Set our timer
            SimDriver.GetPlayerAircraft()->FlareBurstInterval =
                (VU_TIME)(SimLibElapsedTime +
                          (OTWDriver.pCockpitManager->mpIcp->fFLARE_SI
                               [SimDriver.GetPlayerAircraft()->EWSProgNum] *
                           CampaignSeconds));
            //Mark us with one less to go
        ...
        if (SimLibElapsedTime >=
                SimDriver.GetPlayerAircraft()->ChaffBurstInterval and
            ((unsigned int)SimDriver.GetPlayerAircraft()->ChaffCount <
             OTWDriver.pCockpitManager->mpIcp
                 ->iCHAFF_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum]))
        {
        ...
        if (SimLibElapsedTime >=
                SimDriver.GetPlayerAircraft()->FlareBurstInterval and
            ((unsigned int)SimDriver.GetPlayerAircraft()->FlareCount <
             OTWDriver.pCockpitManager->mpIcp
                 ->iFLARE_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum]))
        {
            SimDriver.GetPlayerAircraft()->EWSFlareBurst();
            FlareCheck = FALSE;
        }

        if (SimDriver.GetPlayerAircraft()->FlareCount ==
                OTWDriver.pCockpitManager->mpIcp
                    ->iFLARE_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] and
            SimDriver.GetPlayerAircraft()->ChaffCount ==
                OTWDriver.pCockpitManager->mpIcp
                    ->iCHAFF_BQ[SimDriver.GetPlayerAircraft()->EWSProgNum] and
            SimDriver.GetPlayerAircraft()->ChaffSalvoCount <= 0 and
            SimDriver.GetPlayerAircraft()->FlareSalvoCount <= 0)
        {
            InEWSLoop = FALSE;
        }
    }
    ...
    if (OTWDriver.pCockpitManager->mpIcp->EWS_JAMMER_ON)
    {
        if (HasActivity())
        {
            ...
            if (SimDriver.GetPlayerAircraft()->HasSPJamming() and
                not SimDriver.GetPlayerAircraft()->mFaults->GetFault(
                    FaultClass::blkr_fault) and
                not SimDriver.GetPlayerAircraft()->ManualECM)
            {
                        ...
                        AircraftClass::EWSJammerPower))
                {
                    ...
                    {
                        //Only if fully auto
                        if (SimDriver.GetPlayerAircraft()->EWSPGM() ==
                            AircraftClass::Auto)
                        {
                            SimDriver.GetPlayerAircraft()->SetFlag(ECM_ON);
                        }

                        else if (SimDriver.GetPlayerAircraft()->EWSPGM() ==
                                 AircraftClass::
                                     Semi) //ask if we want turn it on
                        {
                            if (not SaidJammer)
                            {
        ...
        {
            if (not SimDriver.GetPlayerAircraft()->ManualECM and
                SimDriver.GetPlayerAircraft()->EWSPGM() == AircraftClass::Auto)
            {
                // Can't turn off ECM w/ ECM pod broken
                if (not SimDriver.GetPlayerAircraft()->mFaults->GetFault(
                        FaultClass::epod_fault))
...
BOOL PlayerRwrClass::IsOn()
{
    ...
    return (((AircraftClass*)platform)->HasPower(AircraftClass::RwrPower) and
            VehRwrClass::IsOn());
}

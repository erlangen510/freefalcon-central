...
void MissileClass::SetTarget(SimObjectType* newTarget)
{
    ...
    if (newTarget)
    {
        if (launchState == Launching or launchState == InFlight)
        {
            ...
            // #47 UAF ROOT: the missile needs ITS OWN copy of the target node. In Debug the DEBUG branch was empty ->
            // Copy() was skipped -> the missile shared another's node (mutating its localData) -> UAF on
            // freeing the original. Copy UNCONDITIONALLY.

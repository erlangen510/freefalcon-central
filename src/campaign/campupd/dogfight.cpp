...
float startHeading[5] = {0.0F, -45.0F * DTR, 45.0F * DTR, 135.0F * DTR,
                         -135.0F * DTR};
...
void DogfightClass::ApplySettingsToFlight(Flight flight)
{
    ...
    int p, i, cid = 0;
    CampaignTime ttc =
        FloatToInt32(startRange / 1500.0F) *
        CampaignSeconds; // find time to convergence in campaign time
    ...
    if (FalconLocalGame and FalconLocalGame->IsLocal())
    {
        ...
        // Repeat waypoint
        w = flight->AddUnitWP(0, 0, 0, 0, startTime + ttc, 3 * CampaignHours,
                              WP_CA);
        w->SetLocation(x, y, startAltitude);
        ...
        flight->UseFuel(1);
        flight->LoadWeapons(NULL, DefaultDamageMods, Air, numRadarMissiles,
                            WEAP_FORCE_ON_ONE, WEAP_RADAR);
        flight->LoadWeapons(NULL, DefaultDamageMods, Air, numAllAspectMissiles,
                            WEAP_FORCE_ON_ONE, WEAP_HEATSEEKER);
        flight->LoadWeapons(NULL, DefaultDamageMods, Air, numRearAspectMissiles,
                            WEAP_FORCE_ON_ONE,
                            WEAP_HEATSEEKER bitor WEAP_REAR_ASPECT);
...
void DogfightClass::SendSettings(FalconSessionEntity *target)
{
    ...
    if (FalconLocalGame) // and FalconLocalGame->IsLocal()) // We should send the settings even if we are a client and not a host - RH
    {
        ...
        memcpy(&settings->dataBlock.settings.gameType, &SimDogfight.gameType,
               sizeof(DogfightType));
        memcpy(&settings->dataBlock.settings.gameStatus,
               &SimDogfight.gameStatus, sizeof(DogfightStatus));
        memcpy(&settings->dataBlock.settings.startTime, &SimDogfight.startTime,
               sizeof(CampaignTime));
        memcpy(&settings->dataBlock.settings.startRange,
               &SimDogfight.startRange, sizeof(float));
        memcpy(&settings->dataBlock.settings.startAltitude,
               &SimDogfight.startAltitude, sizeof(float));
        memcpy(&settings->dataBlock.settings.xRatio, &SimDogfight.xRatio,
               sizeof(float));
        memcpy(&settings->dataBlock.settings.yRatio, &SimDogfight.yRatio,
               sizeof(float));
        memcpy(&settings->dataBlock.settings.flags, &SimDogfight.flags,
               sizeof(short));
        memcpy(&settings->dataBlock.settings.rounds, &SimDogfight.rounds,
               sizeof(uchar));
        memcpy(&settings->dataBlock.settings.currentRound,
               &SimDogfight.currentRound, sizeof(uchar));
        memcpy(&settings->dataBlock.settings.numRadarMissiles,
               &SimDogfight.numRadarMissiles, sizeof(uchar));
        memcpy(&settings->dataBlock.settings.numRearAspectMissiles,
               &SimDogfight.numRearAspectMissiles, sizeof(uchar));
        memcpy(&settings->dataBlock.settings.numAllAspectMissiles,
               &SimDogfight.numAllAspectMissiles, sizeof(uchar));
...
void DogfightClass::ReceiveSettings(DogfightClass *tmpSettings)
{
    ...
    memcpy(&SimDogfight.gameStatus, &tmpSettings->gameStatus,
           sizeof(DogfightStatus));
    memcpy(&SimDogfight.startTime, &tmpSettings->startTime,
           sizeof(CampaignTime));
    memcpy(&SimDogfight.startRange, &tmpSettings->startRange, sizeof(float));
    memcpy(&SimDogfight.startAltitude, &tmpSettings->startAltitude,
           sizeof(float));
    ...
    memcpy(&SimDogfight.currentRound, &tmpSettings->currentRound,
           sizeof(uchar));
    memcpy(&SimDogfight.numRadarMissiles, &tmpSettings->numRadarMissiles,
           sizeof(uchar));
    memcpy(&SimDogfight.numRearAspectMissiles,
           &tmpSettings->numRearAspectMissiles, sizeof(uchar));
    memcpy(&SimDogfight.numAllAspectMissiles,
           &tmpSettings->numAllAspectMissiles, sizeof(uchar));
...
void DogfightClass::RequestSettings(FalconGameEntity *game)
{
    ...
    if (game)
    {
        VuTargetEntity *target =
            (VuTargetEntity *)vuDatabase->Find(game->OwnerId());
...
void DogfightClass::UpdateDogfight(void)
{
    ...
    if (vuxRealTime > lastUpdateTime + VU_TICS_PER_SECOND)
    {
        ...
        if (localGameStatus == dog_EndRound)
        {
            ...
            if ((restart_matchplay_round) and
                (vuxRealTime > static_cast<VU_TIME>(restart_matchplay_round)))
            {
        ...
        else
        {
            ...
            if ((localFlags bitand DF_VIEWED_SCORES) and
                (gameStatus == dog_Waiting))
            {
                RestartGame();
            }

            if ((FalconLocalSession->GetFlyState() >= FLYSTATE_LOADING) and
        ...
        if ((not SimDriver.GetPlayerEntity()) and (not action_cam_started) and
            (not action_cam_time))
        {
        ...
        if ((action_cam_started) and
            (static_cast<VU_TIME>(action_cam_time) > vuxRealTime))
        {
...
int DogfightClass::AdjustClassId(int oldid, int team)
{
    ...
    return GetClassID(classPtr->vuClassData.classInfo_[0],
                      classPtr->vuClassData.classInfo_[1],
                      classPtr->vuClassData.classInfo_[2],
                      classPtr->vuClassData.classInfo_[3],
                      classPtr->vuClassData.classInfo_[4], team, VU_ANY,
                      VU_ANY);
}
...
void DogfightClass::RegenerateAvailableAircraft(void)
{
    ...
    if (regenerationQueue)
    {
        ...
        while (theObject)
        {
            ...
            if (vuxGameTime > static_cast<VU_TIME>(theObject->timeOfDeath +
                                                   REGEN_WAIT_TIME) or
                gameType == dog_TeamMatchplay)
            {
                if (theObject not_eq FalconLocalSession->GetPlayerEntity() or
                    IsSetLocalFlag(DF_PLAYER_REQ_REGEN) or
                    gameType == dog_TeamMatchplay)
                {
                    msg = new FalconRegenerationMessage(theObject->Id(),
                                                        FalconLocalGame);
                    ...
                    if (theObject == FalconLocalSession->GetPlayerEntity())
                    {
                        ...
                        OTWDriver.SetFrontTextFlags(
                            OTWDriver.GetFrontTextFlags() bitand
                            compl SHOW_DOGFIGHT_SCORES);
                    }

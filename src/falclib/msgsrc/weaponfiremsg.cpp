...
FalconWeaponsFire::FalconWeaponsFire(VU_ID entityId, VuTargetEntity* target,
                                     VU_BOOL loopback)
    : FalconEvent(WeaponFireMsg, FalconEvent::SimThread, entityId, target,
                  loopback)
{
...
FalconWeaponsFire::FalconWeaponsFire(VU_MSG_TYPE type, VU_ID senderid,
                                     VU_ID target)
    : FalconEvent(WeaponFireMsg, FalconEvent::SimThread, senderid, target)
{
    // Your Code Goes Here
    type;
}
...
int FalconWeaponsFire::Process(uchar autodisp)
{
    ...
    if (theEntity)
    {
        if (theEntity->IsSim())
        {
                ...
                dataBlock.weaponType == GUN)
            {
                ...
                if (dataBlock.fWeaponUID.num_ == 0)
                    simEntity->SetFiring(FALSE);
                else
                    simEntity->SetFiring(TRUE);
            }
            ...
            if ((theEntity->IsAirplane() or theEntity->IsHelicopter()) and
                dataBlock.weaponType not_eq FalconWeaponsFire::GUN)
            {
                FalconRadioChatterMessage* radioMessage =
                    new FalconRadioChatterMessage(simEntity->Id(),
                                                  FalconLocalSession);
                ...
                radioMessage->dataBlock.voice_id =
                    ((FlightClass*)(((AircraftClass*)theEntity)
                                        ->GetCampaignObject()))
                        ->GetPilotVoiceID(((AircraftClass*)theEntity)
                                              ->GetCampaignObject()
                                              ->GetComponentIndex(
                                                  ((AircraftClass*)theEntity)));
                ...
                if (WeaponDataTable[GetWeaponIdFromDescriptionIndex(
                                        dataBlock.fWeaponID -
                                        VU_LAST_ENTITY_TYPE)]
                            .Index == 227 or
                    WeaponDataTable[GetWeaponIdFromDescriptionIndex(
                                        dataBlock.fWeaponID -
                                        VU_LAST_ENTITY_TYPE)]
                            .Index == 228)
                {
                    ...
                    radioMessage->dataBlock.edata[0] =
                        ((SimMoverClass*)theEntity)->vehicleInUnit;
                    ...
                    if (theTarget) // we have a target - how far away?
                    {
                        ...
                        float distance =
                            (float)sqrt(dx * dx + dy * dy + dz * dz) * FT_TO_NM;
                        ...
                        radioMessage->dataBlock.edata[1] = 0; // maddog
                }
                else
                {
                    ...
                    if (dataBlock.weaponType == FalconWeaponsFire::Rocket)
                        radioMessage->dataBlock.edata[0] =
                            887; // 2001-09-16 M.N. "Rockets" EVAL INDEX = 887, NOT 163
                    else
                        radioMessage->dataBlock.edata[0] =
                            WeaponDataTable[GetWeaponIdFromDescriptionIndex(
                                                dataBlock.fWeaponID -
            ...
            if (theTarget /* and theTarget->IsSim()*/ and
                dataBlock.weaponType < 3 and
                (theTarget->IsAirplane() or theTarget->IsHelicopter()) and
                (GetTTRelations(theTarget->GetTeam(), simEntity->GetTeam()) <
                 Hostile))
            {
                ...
                if (dataBlock.weaponType == GUN)
                {
                    ...
                    // Check rel geom
                    CalcRelAzEl(simEntity, theTarget->XPos(), theTarget->YPos(),
                                theTarget->ZPos(), &az, &el);
                ...
                if (playMessage and theTarget)
                {
                    ...
                    if (not((AircraftClass*)theTarget)->GetCampaignObject())
                        radioMessage->dataBlock.voice_id =
                            (uchar)((SimBaseClass*)theTarget)
                                ->GetPilotVoiceId();
                    else
                        radioMessage->dataBlock.voice_id =
                            ((FlightClass*)(((AircraftClass*)theTarget)
                                                ->GetCampaignObject()))
                                ->GetPilotVoiceID(
                                    ((AircraftClass*)theTarget)
                                        ->GetCampaignObject()
                                        ->GetComponentIndex(
                                            ((AircraftClass*)theTarget)));
            ...
            if (dataBlock.fWeaponUID not_eq FalconNullId and
                simEntity->GetCampaignObject() and
                ((simEntity->GetCampaignObject())->InPackage() or
                 FalconLocalGame->GetGameType() == game_InstantAction or
                 g_bLogEvents))
        ...
        else
        {
            ...
            if (dataBlock.fWeaponUID not_eq FalconNullId and
                theEntity->IsCampaign() and
                (((CampEntity)theEntity)->InPackage() or
                 FalconLocalGame->GetGameType() == game_InstantAction or
                 g_bLogEvents))
                TheCampaign.MissionEvaluator->RegisterShot(this);
        }
    ...
    if (TheCampaign.MissionEvaluator and
            dataBlock.targetId == FalconLocalSession->GetPlayerEntityID() or
        g_bLogEvents)
        if (theTarget and theTarget->IsSetFalcFlag(FEC_HASPLAYERS) and
            theTarget->IsSim() and
            ((SimBaseClass*)theTarget)->GetCampaignObject())
            TheCampaign.MissionEvaluator->RegisterShotAtPlayer(
                this,
                ((SimBaseClass*)theTarget)->GetCampaignObject()->GetCampID(),
                ((SimMoverClass*)theTarget)->pilotSlot); //me123 added
    ...
    {
        SimBaseClass* theMissile =
            (SimBaseClass*)(vuDatabase->Find(dataBlock.fWeaponUID));
        ...
        if (theTarget and theMissile and theTarget->IsSim())
        {
            ...
            if (simEntity and not simEntity->OnGround())
                ((SimBaseClass*)theTarget)
                    ->SetThreat(simEntity, THREAT_MISSILE);
        }
    }
    ...
    if (dataBlock.weaponType == SRM or dataBlock.weaponType == MRM or
        dataBlock.weaponType == AGM or dataBlock.weaponType == BMB)
    {
        gRebuildBubbleNow = 2;
    }

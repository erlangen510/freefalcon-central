...
FalconATCCmdMessage::FalconATCCmdMessage(VU_ID entityId, VuTargetEntity *target,
                                         VU_BOOL loopback)
...
FalconATCCmdMessage::FalconATCCmdMessage(VU_MSG_TYPE type, VU_ID senderid,
                                         VU_ID target)
...
int FalconATCCmdMessage::Process(uchar autodisp)
{
    ...
    float finalX = 0.0F, finalY = 0.0F, baseX = 0.0F, baseY = 0.0F, x = 0.0F,
          y = 0.0F, z = 0.0F, dx = 0.0F, dy = 0.0F, distD = 0.0F, distA = 0.0F;
    GridIndex X = 0, Y = 0;
    ...
    {
        if (not aircraft->IsPlayer() or
            aircraft->AutopilotType() not_eq AircraftClass::CombatAP)
        {
    ...
    if (aircraft and atc and aircraft->IsAirplane() and atc->IsObjective())
    {
        ...
        if (acBrain and atcBrain)
        {
             ...
             */
            acBrain->SetRunwayInfo(dataBlock.from, dataBlock.rwindex,
                                   dataBlock.rwtime);
            ...
            case Hold:
                ...
                radioMessage = CreateCallFromATC(atc, aircraft, rcATCORBIT2,
                                                 FalconLocalSession);
            ...
            case ToFirstLeg:
                ...
                if (acBrain->ATCStatus() < lFirstLeg)
                {
                    ...
                    atcBrain->FindFinalPt(aircraft, dataBlock.rwindex, &finalX,
                                          &finalY);
                    cosAngle = atcBrain->DetermineAngle(
                        aircraft, dataBlock.rwindex, lFirstLeg);

                    if (cosAngle < 0.0F)
                    {
                        atcBrain->FindBasePt(aircraft, dataBlock.rwindex,
                                             finalX, finalY, &baseX, &baseY);
                        status = atcBrain->FindFirstLegPt(
                            aircraft, dataBlock.rwindex, acBrain->RwTime(),
            ...
            case ToBase:
                ...
                if (acBrain->ATCStatus() < lToBase)
                {
                    atcBrain->FindFinalPt(aircraft, dataBlock.rwindex, &finalX,
                                          &finalY);
                    atcBrain->FindBasePt(aircraft, dataBlock.rwindex, finalX,
                                         finalY, &baseX, &baseY);
                    acBrain->SetATCStatus(lToBase);
                    acBrain->SetTrackPoint(
                        baseX, baseY, atcBrain->GetAltitude(aircraft, lToBase));
                    acBrain->CalculateNextTurnDistance();
                }
            ...
            case ToFinal:
                ...
                if (acBrain->ATCStatus() < lToFinal)
                {
                    ...
                    acBrain->SetTrackPoint(
                        finalX, finalY,
                        atcBrain->GetAltitude(aircraft, lToFinal));
                    acBrain->CalculateNextTurnDistance();
                }
            ...
            case OnFinal:
                ...
                acBrain->SetTrackPoint(
                    x, y, atcBrain->GetAltitude(aircraft, lOnFinal));

                if (aircraft->IsPlayer())
                {
                    //acBrain->GetTrackPoint(&x, &y, &desAlt);
                    radioMessage = CreateCallFromATC(
                        atc, aircraft, rcTURNTOFINAL, FalconLocalSession);

                    float rnwyHdg, planeHdg;

                    rnwyHdg =
                        PtHeaderDataTable[dataBlock.rwindex].data + 180.0F;
            ...
            case EmerToBase:
                ...
                if (acBrain->ATCStatus() not_eq lEmergencyToBase)
                {
                    ...
                    acBrain->SetTrackPoint(
                        baseX, baseY,
                        atcBrain->GetAltitude(aircraft, lEmergencyToBase));
            ...
            case EmerToFinal:
                ...
                if (acBrain->ATCStatus() not_eq lEmergencyToFinal)
                {
                    ...
                    acBrain->SetTrackPoint(
                        finalX, finalY,
                        atcBrain->GetAltitude(aircraft, lEmergencyToFinal));
            ...
            case EmerOnFinal:
                ...
                {
                    acBrain->SetTaxiPoint(GetFirstPt(dataBlock.rwindex));
                    TranslatePointData(atc, GetFirstPt(dataBlock.rwindex), &x,
                                       &y);
                    acBrain->SetATCStatus(lEmergencyOnFinal);
                    acBrain->SetTrackPoint(
                        x, y,
                        atcBrain->GetAltitude(aircraft, lEmergencyOnFinal));
                }
            ...
            case Abort:
                ...
                {
                    radioMessage = CreateCallFromATC(
                        atc, aircraft, rcATCGOAROUND2, FalconLocalSession);
                    //M.N. changed to 32767 -> flexibly use randomized value of max available eval indexes
                ...
                {
                    radioMessage = CreateCallFromATC(
                        atc, aircraft, rcATCGOAROUND, FalconLocalSession);

                    atcBrain->CalculateStandRateTurnToPt(aircraft, x, y,
                                                         &finalHdg);
                    radioMessage->dataBlock.edata[3] =
                        (short)FloatToInt32(finalHdg);
                ...
                if (atc->IsLocal())
                {
                    atcBrain->RemoveTraffic(
                        aircraft->Id(),
                        PtHeaderDataTable[dataBlock.rwindex].runwayNum);
                }
            ...
            case ClearToLand:
                ...
                radioMessage = CreateCallFromATC(atc, aircraft, rcCLEAREDLAND,
                                                 FalconLocalSession);
                radioMessage->dataBlock.edata[4] =
                    (short)atcBrain->GetRunwayName(
                        atcBrain->GetOppositeRunway(dataBlock.rwindex));
                FalconSendMessage(radioMessage, FALSE);
            ...
            case Taxi:
                ...
                {
                case RunwayPt:
                    atcBrain->FindRunwayPt(
                        (Flight)aircraft->GetCampaignObject(),
                        aircraft->vehicleInUnit, dataBlock.rwindex, &x, &y);
                    break;

                case TakeoffPt:
                    atcBrain->FindTakeoffPt(
                        (Flight)aircraft->GetCampaignObject(),
                        aircraft->vehicleInUnit, dataBlock.rwindex, &x, &y);
                    break;
                ...
                acBrain->SetTrackPoint(x, y,
                                       atcBrain->GetAltitude(aircraft, tTaxi));
            ...
            case TakeRunway:
                ...
                if (not acBrain->isWing or aircraft->IsPlayer() or
                    aircraft->vehicleInUnit == 2 or
                    not atcBrain->UseSectionTakeoff(
                        (Flight)aircraft->GetCampaignObject(),
                        dataBlock.rwindex))
                {
                    radioMessage = CreateCallFromATC(
                        atc, aircraft, rcPOSITIONANDHOLD, FalconLocalSession);
                    radioMessage->dataBlock.edata[3] =
                        (short)atcBrain->GetRunwayName(dataBlock.rwindex);
                    FalconSendMessage(radioMessage, FALSE);
                }
            ...
            case Takeoff:
                        ...
                        dataBlock.rwindex))
                {
                    ...
                    {
                        radioMessage =
                            CreateCallFromATC(atc, aircraft, rcCLEAREDONRUNWAY,
                                              FalconLocalSession);
                        radioMessage->dataBlock.edata[3] =
                            (short)atcBrain->GetRunwayName(dataBlock.rwindex);
                        FalconSendMessage(radioMessage, FALSE);
                    ...
                    {
                        SendCallFromATC(atc, aircraft, rcCLEAREDDEPARTURE,
                                        FalconLocalSession);
                    }
                }
            ...
            case Divert:
...

                {
                    ...
                    divertBase =
                        FindNearestFriendlyAirbase(aircraft->GetTeam(), X, Y);
                    ...
                    if (not divertBase)
                    {
                        divertBase = FindNearestFriendlyRunway(
                            aircraft->GetTeam(), X, Y);
                    }
                    ...
                    if (divertBase)
                    {
                        if (divertBase->GetType() == TYPE_AIRBASE)
                        {
                            ...
                            rangediv =
                                NM_TO_KM *
                                ((float)sqrt(diffx * diffx + diffy * diffy) *
                                 FT_TO_NM);
                            ...
                            radioMessage = CreateCallFromATC(
                                atc, aircraft, rcATCDIVERT, FalconLocalSession);
                            radioMessage->dataBlock.edata[3] =
                                (short)divertBase->brain->Name();
                            radioMessage->dataBlock.edata[4] =
                                (short)az /*SimToGrid(divertBase->YPos())*/;
                            radioMessage->dataBlock.edata[5] = (short)
                                rangediv /*SimToGrid(divertBase->XPos())*/;
                            gTacanList->GetChannelFromVUID(
                                divertBase->Id(), &channel, &station, &domain,
                                &range, &ttype, &ilsfreq);

                            radioMessage->dataBlock.edata[6] =
                                (short)(channel -
                                        g_nMinTacanChannel); //right now Joe has this using the eBearing eval
                            //but this will be replaced by a channel eval
                            radioMessage->dataBlock.edata[7] =
                                (short)(station +
                                        23); //this uses the eAlfabet eval and stations are
                            //either X or Y
                        }
                        else
                        {
                        ...
                        FalconSendMessage(radioMessage, FALSE);
                        acBrain->SetRunwayInfo(divertBase->Id(), 0,
                                               SimLibElapsedTime);
                        acBrain->SetTrackPoint(divertBase->XPos(),
                                               divertBase->YPos(), -5000.0F);
                        acBrain->CalculateNextTurnDistance();
                    }
                    else
                    {
                        SendCallFromATC(atc, aircraft, rcUSEALTFIELD,
                                        FalconLocalSession);
                    }
            ...
            case Release:
                ...
                //rcDEPARTHEADING (7)
                //rcRESUMEOWNNAV (5)
                radioMessage = CreateCallFromATC(atc, aircraft, rcDEPARTHEADING,
                                                 FalconLocalSession);
                radioMessage->dataBlock.edata[3] =
                    atcBrain->FindBestLandingRunway(aircraft, TRUE);
                FalconSendMessage(radioMessage, FALSE);
                break;
